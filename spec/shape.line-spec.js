...
describe("SHAPE LINE", () => {
	...
	describe("stacked area when line.connectNull=true", () => {
		before(() => {
			args = {
				data: {
					columns: [
		...
		it("check for area path data count", () => {
			chart.internal.main.selectAll("path.bb-area").each(function(d) {
				const area = d3.select(this);

				// it should have: 9 for data1 and 8 for else
				const pathLineCount = area.classed("bb-area-data1") ? 9 : 8;

				expect(area.attr("d").split("L").length).to.be.equal(pathLineCount);
			});
		});
	});
});
