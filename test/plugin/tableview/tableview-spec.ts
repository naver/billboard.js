...
describe("PLUGIN: TABLE-VIEW", () => {
	...
	describe("isValidTableViewOptions", () => {
		...
		it("constructor should not throw when given invalid options (warns and falls back to defaults)", () => {
			// supported option with a value of the wrong type
			expect(() => new TableView({style: "true"} as any)).to.not.throw();
			// non-object value
			expect(() => new TableView("nope" as any)).to.not.throw();
		});

		it("constructor should warn when given invalid options", () => {
			const spy = sinon.spy(console, "error");

			new TableView({style: "true"} as any);
