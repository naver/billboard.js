...
import { scaleSequential, scaleSequentialLog, scaleSymlog } from "d3-scale";
...
const getRange = (start, end, step = 1) => {
	...
	for (let i = 0; i < n; i++) res.push(start + i * step);
	return res;
};
/**
...
function parseDate(date) {
	...
	} else if (isNumber(date) && !isNaN(date)) parsedDate = /* @__PURE__ */ new Date(+date);
...
var Plugin = class {
	...
	options;
	config;
	static version = "4.0.3-nightly-20260903010615";
	/**
	* Constructor
...
//#endregion
//#region src/Plugin/stanford/classes.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
//#endregion
//#region src/Plugin/stanford/util.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
* @ignore
...
//#endregion
//#region src/Plugin/stanford/ColorScale.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
function format(specifier) {
	if (specifier === "d") return (n) => Math.round(n).toString();
	return (n) => String(n);
}
/**
...
var ColorScale = class {
	...
	drawColorScale() {
		...
		const inverseScale = scaleSequential(target.colors).domain([points[points.length - 1], points[0]]);
		...
		else if (isFunction(scaleFormat)) legendAxis.tickFormat(scaleFormat);
		else legendAxis.tickFormat(format("d"));
		const axis = this.colorScale.append("g").attr("class", "legend axis").attr("transform", `translate(${barWidth},0)`).call(legendAxis);
		if (scaleFormat === "pow10") axis.selectAll(".tick text").text(null).filter((d) => d / Math.pow(10, Math.ceil(Math.log(d) / Math.LN10 - 1e-12)) === 1).text(10).append("tspan").attr("dy", "-.7em").text((d) => Math.round(Math.log(d) / Math.LN10));
...
//#endregion
//#region src/Plugin/stanford/Elements.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
var Elements = class {
	...
	constructor(owner) {
		...
		elements.append("g").attr("class", classes_default.stanfordLines);
		elements.append("g").attr("class", classes_default.stanfordRegions);
	}
	updateStanfordLines(duration) {
		...
		const stanfordLine = main.select(`.${classes_default.stanfordLines}`).style("shape-rendering", "geometricprecision").selectAll(`.${classes_default.stanfordLine}`).data(this.owner.config.lines);
		...
		stanfordLineEnter.merge(stanfordLine).attr("class", (d) => classes_default.stanfordLine + (d.class ? ` ${d.class}` : "")).select("line").transition().duration(duration).attr("x1", (d) => {
	...
	updateStanfordRegions(duration) {
		...
		let stanfordRegion = main.select(`.${classes_default.stanfordRegions}`).selectAll(`.${classes_default.stanfordRegion}`).data(this.owner.config.regions);
	...
	xvCustom(d, xyValue) {
		...
		else if (axis.isCategorized() && isString(value)) value = config.axis_x_categories.indexOf(d.value);
		return $$.scale.x(value);
	}
	yvCustom(d, xyValue) {
		const $$ = this;
		return (d.axis && d.axis === "y2" ? $$.scale.y2 : $$.scale.y)(xyValue ? d[xyValue] : $$.getBaseValue(d));
	}
};
//#endregion
//#region src/Plugin/stanford/Options.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
//#endregion
//#region src/Plugin/stanford/index.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
* })
*/
var Stanford = class extends Plugin {
	constructor(options) {
		super(options);
	...
	initStanfordData() {
		...
		target.colors = isFunction(config.colors) ? config.colors : interpolateHslLong(hsl(250, 1, .5), hsl(0, 1, .5));
		target.colorscale = scaleSequentialLog(target.colors).domain([target.minEpochs, target.maxEpochs]);
	}
	getStanfordPointColor(d) {
		return this.data.targets[0].colorscale(d.epochs);
	}
	setStanfordTooltip() {
		const { config } = this.$$;
		if (isEmpty(config.tooltip_contents)) config.tooltip_contents = function(d, defaultTitleFormat, defaultValueFormat, color) {
			const { data_x } = config;
	...
	countEpochsInRegion(region) {
		...
		const value = target.values.reduce((accumulator, currentValue) => {
			if (pointInRegion(currentValue, region)) return accumulator + Number(currentValue.epochs);
			return accumulator;
		}, 0);
		return {
			value,
