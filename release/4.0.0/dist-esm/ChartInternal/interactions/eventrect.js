 ...
 * @version 4.0.0
*/
import { $SHAPE, $COMMON, $EVENT } from '../../config/classes.js';
import { getPointer, getScrollPosition, getBoundingRect } from '../../module/util/dom.js';
...
var eventrect = {
    ...
    updateEventRectData() {
        ...
        const fingerprint = xDomain ?
            `${xDomain[0]}_${xDomain[1]}_${state.width}_${state.height}_${$$.data.targets.length}_${state.dataGeneration}_${[...state.hiddenTargetIds].join(",")}` :
            null;
    ...
    selectRectForSingle(context, index) {
        ...
        !circle &&
            main.selectAll(`.${$COMMON.EXPANDED}:not(.${$SHAPE.shape}-${index})`).classed($COMMON.EXPANDED, false);
    ...
    selectRectForMultipleXs(context, triggerEvent = true) {
        ...
        const sameXData = ($$.isBubbleType(closest) || $$.isScatterType(closest) || !config.tooltip_grouped) ?
            [closest] :
        ...
        if (state.inputType === "mouse") {
            ...
            rect
                ...
                Object.values($$.$el.axisTooltip)
                    .forEach((v) => v?.style("display", null));
            })
