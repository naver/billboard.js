...
import { hasCanvasDrawableValue, isCanvasTargetSupported, isCanvasPointType, isCanvasBarType, isCanvasCandlestickType, isCanvasTreemapType, isCanvasScatterType, isCanvasBubbleType } from '../../canvas/util.js';
...
import { $LEGEND, $FOCUS } from '../classes.js';
import { window as win } from '../../module/browser.js';
import { KEY } from '../../module/Cache.js';
import { extend, callFn, tplProcess, parseDate } from '../../module/util/object.js';
import { getBoundingRect } from '../../module/util/dom.js';
import { isFunction, isBoolean } from '../../module/util/type-checks.js';
...
const canvasSelectionDragRows = new WeakMap();
/**
...
function getCanvasLegendPointPattern($$, id) {
    ...
    const pattern = $$.getValidPointPattern?.() || (Array.isArray(config.point_pattern) && config.point_pattern.length ?
...
function setCanvasHtmlLegendItem($$, item) {
    ...
    item
        ...
        const current = select(this).attr("class") || "";
...
function setCanvasHtmlLegendFocus($$, id) {
    ...
    legend?.selectAll(`.${$LEGEND.legendItem}`)
        .classed($FOCUS.legendItemFocused, (d) => targetIds.indexOf(d) >= 0)
...
function getCanvasSelectionDragDelta($$, dataRows) {
    ...
    const previousRows = canvasSelectionDragRows.get($$) || new Map();
...
const canvasInternal = {
    ...
    flushCanvasFlow() {
        ...
        state.canvasFlowFrame !== null &&
            win.cancelAnimationFrame?.(state.canvasFlowFrame);
        state.canvasFlowFrame = null;
    ...
    updateHtmlLegend() {
        ...
        const item = enter.merge(legendItems)
            .attr("class", id => $$.generateClass($LEGEND.legendItem, id).trim());
        setCanvasHtmlLegendItem($$, item);
        item.select(`.${$LEGEND.legendItemTile}`)
            .classed($CANVAS.legendItemTileCircle, !config.legend_usePoint && config.legend_item_tile_type === "circle")
    ...
    updateHtmlLegendSize(targetIds) {
        ...
        if (useTemplateLegend) {
            ...
            const measured = targetIds.map((id, index) => {
                ...
                const width = hidden ? 0 : ((rect?.width || fallbackWidth) +
                    dimension.tileWidth +
            ...
            const xForLegend = state.isLegendRight ?
                (id) => dimension.max.width * sizes.steps[id] :
                state.isLegendInset ?
                    (id) => dimension.max.width * sizes.steps[id] + 10 :
                    (id) => sizes.margins[sizes.steps[id]] + sizes.offsets[id];
            const yForLegend = state.isLegendRight || state.isLegendInset ?
                (id) => sizes.margins[sizes.steps[id]] + sizes.offsets[id] :
                (id) => dimension.max.height * sizes.steps[id];
    ...
    positionHtmlLegend() {
        ...
        if (state.isLegendRight) {
            ...
            legend
                ...
                .style("width", `${Math.max(0, state.legendItemWidth * (state.legendStep + 1) + 20)}px`);
        }
    },
    /**
    ...
    canStartCanvasSelectionDrag(event) {
        ...
        const { config } = $$;
        const mouse = event;
        return Boolean(config.interaction_enabled &&
    ...
    startCanvasSelectionDrag(event) {
        ...
        state.canvasSelectionDragIncluded = new Set();
        state.canvasSelectionDragging = true;
        state.canvasSelectionDragMoved = false;
        canvasSelectionDragRows.set($$, new Map());
        $$.setDragStatus(true);
        $$.hideTooltip?.();
        event.preventDefault?.();
        return true;
