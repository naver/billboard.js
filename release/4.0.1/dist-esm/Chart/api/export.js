...
import { getBoundingRect, getCssRules, getBBox } from '../../module/util/dom.js';
 ...
 */
const b64EncodeUnicode = (str) => win.btoa?.(encodeURIComponent(str)
    .replace(/%([0-9A-F]{2})/g, (match, p) => String.fromCharCode(Number(`0x${p}`))));
/**
...
function nodeToSvgDataUrl(node, option, orgSize) {
    ...
    const cssText = getCssRules(toArray(doc.styleSheets))
        .filter((r) => r.cssText)
        .map((r) => r.cssText);
