...
import { url, columns, rows, json } from './convert.helper.js';
import { toSet } from '../../module/util/object.js';
import { isUndefined, isDefined, isValue, notEmpty, isObject, isArray, isEmpty } from '../../module/util/type-checks.js';
...
var dataConvert = {
    ...
    convertData(args, callback) {
        ...
        if (data.url && callback) {
            url(data.url, data.mimeType, data.headers, _getDataKeyForJson(data.keys, config), callback);
        }
        else if (data.json) {
            runWorker(useWorker(data.json), json, callback, [columns, rows])(data.json, _getDataKeyForJson(data.keys, config));
        }
        else if (data.rows) {
    ...
    convertDataToTargets(data, appendXs) {
        ...
        const categoryIndexMap = (params.customX && params.categorized && config.axis_x_categories.length) ?
            new Map(config.axis_x_categories.map((cat, i) => [cat, i])) :
            null;
        // convert to target
