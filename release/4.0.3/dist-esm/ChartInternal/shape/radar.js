...
import { $AXIS, $LEVEL, $TEXT, $COMMON, $RADAR, $SHAPE } from '../../config/classes.js';
import { KEY } from '../../module/Cache.js';
import { setTextValue, getBoundingRect, getPathBox } from '../../module/util/dom.js';
import { isUndefined, isEmpty, isNumber, isDefined } from '../../module/util/type-checks.js';
import { getRange, toArray, getMinMax } from '../../module/util/object.js';
...
var shapeRadar = {
    ...
    getRadarPosition(type, index, range, ratio) {
        ...
        const pos = toArray(type).map(v => _getPosition(isClockwise, v, edge, index, isDefined(range) ? range : (type === "x" ? width : height), isNumber(ratio) ? ratio : config.radar_size_ratio));
    ...
    generateRadarPoints() {
        ...
        if (!size || (size.width !== width && size.height !== height)) {
            targets.forEach(d => {
                points[d.id] = d.values.map((v, i) => ($$.getRadarPosition(["x", "y"], i, undefined, $$.getRatio("radar", v))));
            });
    ...
    updateRadarLevel() {
        ...
        const points = levelData.map(v => {
            ...
            const pos = getRange(0, edge).map(i => ($$.getRadarPosition(["x", "y"], i, range, 1)).join(","));
            return pos.join(" ");
        });
