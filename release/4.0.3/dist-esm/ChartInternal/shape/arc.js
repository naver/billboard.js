...
import { $COMMON, $ARC, $GAUGE, $FOCUS } from '../../config/classes.js';
import { document as doc } from '../../module/browser.js';
import { isLabelWithLine, redrawArcLabelLines } from '../internals/text.arc.js';
import { updateTextImage, meetsLabelThreshold, updateTextImagePos } from '../internals/text.util.js';
import { isUndefined, isFunction, isNumber, isDefined, isObject } from '../../module/util/type-checks.js';
import { endall, callFn, tplProcess } from '../../module/util/object.js';
...
function _getRadiusFn(expandRate = 0) {
    ...
    return {
        ...
        corner(d, outerRadius) {
            const { arc_cornerRadius_ratio: ratio = 0, arc_cornerRadius: cornerRadius = 0 } = config;
...
function _getAttrTweenFn(fn) {
    return function (d) {
        ...
        const interpolate$1 = interpolate(getAngleKeyValue(this._current), getAngleKeyValue(d));
        this._current = d;
        return function (t) {
            const interpolated = interpolate$1(t);
...
var shapeArc = {
    initPie() {
        ...
        $$.pie = pie()
            ...
            .padAngle(padAngle)
            .value((d) => d.values?.reduce((a, b) => a + b.value, 0) ?? d)
    ...
    getSvgArcExpanded(rate = 1) {
        ...
        return (d) => {
            ...
            return updated ? arc$1.cornerRadius(cornerR)(updated) || "M 0 0" : "M 0 0";
        };
    },
    ...
    redrawArcRangeText() {
        ...
        if (values?.length) {
            ...
            rangeText.style("opacity", d => (!fixed && (d > totalSum || totalSum === 0) ? "0" : null));
        }
    },
    /**
    ...
    textForArcLabel(selection) {
        ...
        if ($$.shouldShowArcLabel()) {
            selection
                ...
                .attr("filter", d => $$.updateTextBGColor.bind($$)(d, $$.config.data_labels_backgroundColors))
                .each(function (d) {
    ...
    updateTargetsForArc(targets) {
        ...
        const mainPieEnter = mainPieUpdate.enter().append("g")
            ...
            .call(this.setCssRule(false, `.${$ARC.chartArcs} text`, [
            "pointer-events:none",
            "text-anchor:middle"
        ]));
    ...
    redrawArc(duration, durationForExit, withTransform) {
        ...
        mainArc
            ...
            const interpolate$1 = interpolate(this._current, updated);
    ...
    bindArcEvent(arc) {
        ...
        if (isTouch && $$.hasArcType() && !$$.radars) {
            const getEventArc = event => {
                ...
                const eventArc = select(doc.elementFromPoint(clientX, clientY));
                return eventArc;
            };
            $$.$el.svg
    ...
    redrawArcText(duration) {
        ...
        if (!(hasGauge && $$.data.targets.length === 1 && config.gauge_title)) {
            text = main.selectAll(`.${$ARC.chartArc}`)
                ...
                .style("font-size", d => ($$.isGaugeType(d.data) && $$.data.targets.length === 1 && !hasMultiArcGauge ?
