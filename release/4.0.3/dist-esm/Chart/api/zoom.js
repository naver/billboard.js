...
import { parseDate, extend, getMinMax } from '../../module/util/object.js';
import { isObject, isDefined } from '../../module/util/type-checks.js';
...
const zoom = function (domainValue) {
    ...
    if (config.zoom_enabled) {
        ...
        if (Array.isArray(domain)) {
            ...
            const isWithinRange = $$.withinRange(domain, $$.getZoomDomain("zoom", true), $$.getZoomDomain("zoom"));
            if (isWithinRange) {
...
var apiZoom = {
    ...
    unzoom() {
        ...
        if (zoom || (state.isCanvasMode && config.subchart_show && state.domain)) {
            ...
            // reset transform
            if (target?.node() && zoomTransform(target.node()) !== zoomIdentity) {
                $$.zoom.transform(target, zoomIdentity);
            }
