 ...
 * @version 4.1.0
*/
//#region src/ChartInternal/internals/subchart.util.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
function getMainCoordFromSubchartCoord($$, subCoord) {
	...
	if (!Number.isFinite(mainCoord) && subLength > 0) mainCoord = Math.max(0, Math.min(1, subCoord / subLength)) * mainLength;
	return Number.isFinite(mainCoord) ? Math.max(0, Math.min(mainLength, mainCoord)) : null;
}
//#endregion
export { getMainCoordFromSubchartCoord, isContinuousGridFocusEnabled };
