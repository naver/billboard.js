...
import { hasCanvasDrawableValue, isCanvasPointType, isFiniteCanvasCoordinate, isCanvasAreaType, getCanvasShapeIndices, isCanvasBarType, isCanvasCandlestickType, isCanvasLineType, isCanvasScatterType, isCanvasBubbleType, getCanvasTargetVisibleRange, createCanvasPointOccupancyGrid, markCanvasPointOccupancy, isCanvasTreemapType, getFontSize, isCanvasTargetSupported, DENSE_SCATTER_POINT_CULL_THRESHOLD } from './util.js';
...
class CanvasRenderer {
    ...
    drawAreas($$, shape, focusData) {
        ...
        painter.withTranslation(margin.left, margin.top, () => {
            for (const target of targets) {
                ...
                ctx.fillStyle = getCanvasLinearGradientFill($$, ctx, visibleTarget, "area", 
                ...
                $$.config.area_linearGradient ? getCanvasAreaBounds($$, target, indices) : null, color);
        ...
        painter.withTranslation(margin.left, margin.top, () => {
            for (const target of $$.filterTargetsToShow()) {
                ...
                const occupancy = cullDenseScatter ?
                    createCanvasPointOccupancyGrid($$.state.width, $$.state.height, getTargetPointRadius($$, target, target.values[range.start] || target.values[0])) :
                    null;
    ...
    drawSelections($$, shape) {
        ...
        const barPoints = selectedData.some(isBar) ?
            $$.generateGetBarPoints(getCanvasShapeIndices($$, shape, TYPE.BAR, isBar), false) :
            null;
    ...
    drawLabels($$, shape) {
        ...
        const candlestickPoints = $$.generateGetCandlestickPoints ?
            $$.generateGetCandlestickPoints(getCanvasShapeIndices($$, shape, TYPE.CANDLESTICK, isCanvasCandlestickType.bind(null, $$)), false) :
            null;
        const targets = $$.filterTargetsToShow()
            .filter(target => (isCanvasBarType($$, target) ||
                ...
                for (const { d, text } of rows) {
                    ...
                    if (isCanvasBarType($$, d) && barPoints) {
                        ...
                        y = $$.config.data_labels.centered ?
                            rect.y + rect.h / 2 :
                            ($$.config.axis_rotated ? rect.y + rect.h / 2 : end[1] + (isNegative ? labelHeight - 3 : -3));
    ...
    drawTreemaps($$) {
        ...
        painter.withState(() => {
            ...
            for (const node of nodes) {
                ...
                const fontBoundingHeight = ((metrics.fontBoundingBoxAscent || 0) +
                    (metrics.fontBoundingBoxDescent || 0)) || ((metrics.actualBoundingBoxAscent || 0) +
    ...
    drawFocus($$, selectedData) {
        ...
        const { style } = this.theme;
        const { margin } = $$.state;
        const focus = selectedData.find(d => d &&
            hasCanvasDrawableValue($$, d));
        painter.withTranslation(margin.left, margin.top, () => {
            if ($$.config.tooltip_show &&
                ...
                if (hasIndexCoordinate) {
                    painter.strokePath(() => {
                        if (isRotated) {
                            painter.traceLine(crispEdgeX(0), y, isEdge && hasValueCoordinate ? x : crispEdgeX($$.state.width), y);
                            if (hasValueCoordinate &&
                                $$.config.grid_focus_y &&
                            ...
                            painter.traceLine(x, isEdge && hasValueCoordinate ? y : crispEdgeY(0), x, crispEdgeY($$.state.height));
                            if (hasValueCoordinate &&
                                $$.config.grid_focus_y &&
                                !$$.config.tooltip_grouped) {
                                const isY2 = $$.axis?.getId(focus.id) === "y2";
