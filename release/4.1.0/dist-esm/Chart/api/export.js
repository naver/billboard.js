...
import { mergeObj, toArray } from "../../module/util/object.js";
import { getBBox, getBoundingRect, getCssRules } from "../../module/util/dom.js";
import { namespaces } from "d3-selection";
//#region src/Chart/api/export.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
*/
const b64EncodeUnicode = (str) => win.btoa?.(encodeURIComponent(str).replace(/%([0-9A-F]{2})/g, (match, p) => String.fromCharCode(Number(`0x${p}`))));
/**
...
function nodeToSvgDataUrl(node, option, orgSize) {
	...
	const cssText = getCssRules(toArray(doc.styleSheets)).filter((r) => r.cssText).map((r) => r.cssText);
	clone.setAttribute("xmlns", namespaces.xhtml);
	clone.style.margin = "0";
	clone.style.padding = "0";
	if (option.preserveFontStyle) clone.querySelectorAll("text").forEach((t) => {
		t.innerHTML = "";
	});
	...
	const dataStr = `<svg xmlns="${namespaces.svg}" width="${width}" height="${height}" 
...
function getGlyph(svg) {
	...
	toArray(svg.querySelectorAll("text")).filter(filterFn).forEach((t) => {
		const getStyleFn = (ts) => {
			const { fill, fontFamily, fontSize, textAnchor, transform } = win.getComputedStyle(ts);
			const { x, y, width, height } = getCoords(ts, {
				left,
				top
			});
			return { [ts.textContent]: {
				x,
				y,
				width,
				height,
		...
		if (t.childElementCount > 1) toArray(t.querySelectorAll("tspan")).filter(filterFn).forEach((ts) => {
			glyph.push(getStyleFn(ts));
		});
		else glyph.push(getStyleFn(t));
	});
	return glyph;
}
/**
...
function renderText(ctx, glyph) {
	glyph.forEach((g) => {
		Object.keys(g).forEach((key) => {
			...
			ctx.fillStyle = fill;
			if (transform === "none") ctx.fillText(key, x, y);
			else {
...
export(option, callback) {
	...
	if (state.isCanvasMode && canvas) {
		const dataUrl = canvasToDataUrl(canvas, opt, {
			width,
			height
		});
