...
flow(args) {
	const $$ = this.internal;
	let data;
	if (args.json || args.rows || args.columns) $$.convertData(args, (res) => {
		data = res;
		_();
	});
	/**
	...
	*/
	function _() {
		...
		let diff;
		let to;
		if ($$.state.redrawing || !data || !isTabVisible()) return;
		$$.flushCanvasFlow?.();
		const notfoundIds = [];
		...
		$$.data.targets.forEach((t) => {
			let found = false;
			for (let i = 0; i < targets.length; i++) if (t.id === targets[i].id) {
				found = true;
				if (t.values[t.values.length - 1]) tail = t.values[t.values.length - 1].index + 1;
				const values = targets[i].values;
				length = values.length;
				for (let j = 0; j < length; j++) {
					values[j].index = tail + j;
					if (!isTimeSeries) values[j].x = tail + j;
		...
		$$.data.targets.forEach((t) => {
			for (let i = 0; i < notfoundIds.length; i++) if (t.id === notfoundIds[i]) {
				if (!t.values[t.values.length - 1]) continue;
				tail = t.values[t.values.length - 1].index + 1;
				for (let j = 0; j < length; j++) t.values.push({
					id: t.id,
					index: tail + j,
		...
		if ($$.data.targets.length) targets.forEach((t) => {
			...
			if (missingLength) {
				...
				for (let i = valueLength - 1; i >= 0; i--) values[i + missingLength] = values[i];
			...
			for (let i = missingLength; i < values.length; i++) {
				const v = values[i];
				v.index += tail;
				if (!isTimeSeries) v.x += tail;
			}
		});
		for (let i = 0; i < targets.length; i++) $$.data.targets.push(targets[i]);
		...
		} else if (isDefined(args.length)) length = args.length;
		if (!orgDataCount) {
			if (isTimeSeries) diff = baseTarget.values.length > 1 ? baseTarget.values[baseTarget.values.length - 1].x - baseValue.x : baseValue.x - $$.getXDomain($$.data.targets)[0];
			else diff = 1;
			domain = [baseValue.x - diff, baseValue.x];
		} else if (orgDataCount === 1 && isTimeSeries) {
			diff = (baseTarget.values[baseTarget.values.length - 1].x - baseValue.x) / 2;
			domain = [/* @__PURE__ */ new Date(+baseValue.x - diff), new Date(+baseValue.x + diff)];
		}
		...
		if ($$.state.isCanvasMode) {
			...
			if ($$.animateCanvasFlow?.({
				done: args.done,
				duration,
				length,
				orgDataCount
			})) return;
			if (length && orgDataCount) $$.data.targets.forEach((d) => {
