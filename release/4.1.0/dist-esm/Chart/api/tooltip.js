...
var tooltip_default = { tooltip: {
	...
	*/
	show: function(args) {
		...
		let index;
		let mouse;
		if (args.mouse) mouse = args.mouse;
		if (args.data) {
			const { data } = args;
			...
			} else if ($$.isMultipleX()) mouse = [$$.xx(data), y];
			else {
				if (!config.tooltip_grouped) mouse = [0, y];
				index = data.index ?? ($$.hasArcType() && data.id ? $$.getArcElementByIdOrIndex(data.id)?.datum().index : $$.getIndexByX(data.x));
			}
		} else if (isDefined(args.x)) index = $$.getIndexByX(args.x);
		else if (isDefined(args.index)) index = args.index;
		if ($$.state.isCanvasMode) {
			const targets = $$.filterTargetsToShow?.() || $$.data.targets;
			const selectedData = args.data?.id && !config.tooltip_grouped ? targets.filter((target) => target.id === args.data.id).map((target) => target.values[index ?? args.data.index]).filter(Boolean) : targets.map((target) => target.values[index]).filter(Boolean);
			const canvas = $el.canvas?.node?.();
			const shape = $$.state.canvasShape || $$.getDrawShape?.();
			const first = selectedData[0];
			const point = mouse || (first && shape?.pos?.cx && shape?.pos?.cy ? [$$.state.margin.left + shape.pos.cx(first), $$.state.margin.top + shape.pos.cy(first)] : void 0);
			if (!selectedData.length || !canvas) return;
