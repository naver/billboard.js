 ...
 * @version 4.1.0
*/
import { isDefined, isNumber, isObjectType, isValue } from "../../module/util/type-checks.js";
//#region src/Chart/api/axis.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
function setMinMax($$, type, value) {
	...
	const helper = (key, value) => {
		const v = isNumber(value) ? value : value === false ? void 0 : null;
		if (v !== null) config[`axis_${key}_${type}`] = v;
	};
	if (isDefined(value)) {
		if (isObjectType(value)) Object.keys(value).forEach((key) => {
			helper(key, value[key]);
		});
		else if (isNumber(value) || value === false) ["y", "y2"].forEach((key) => {
			helper(key, value);
		});
...
var axis_default = { axis: {
	...
	*/
	labels: function(labels) {
		...
		if (labels) {
			...
			if ($$.state.isCanvasMode) $$.renderCanvasFrame?.(void 0, null, false);
			else $$.axis.updateLabels();
		}
		[
			"x",
			"y",
	...
	*/
	min: function(min) {
		const $$ = this.internal;
		return isValue(min) || min === false ? setMinMax($$, "min", min) : getMinMax($$, "min");
	},
	/**
	...
	*/
	max: function(max) {
		const $$ = this.internal;
		return isValue(max) || max === false ? setMinMax($$, "max", max) : getMinMax($$, "max");
	},
	/**
...
//#endregion
export { axis_default as default };
