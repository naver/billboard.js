 ...
 * @version 4.1.0
*/
import { isArray, isUndefined } from "../../module/util/type-checks.js";
import { extend } from "../../module/util/object.js";
//#region src/Chart/api/data.ts
/**
	...
	values: function(targetIds, flat = true) {
		...
		if (targetIds) {
			...
			if (isArray(targets)) {
				values = [];
				targets.forEach((v) => {
					if (flat) v.values.forEach((d) => values.push(d.value));
					else values.push(v.values.map((d) => d.value));
				});
			}
		}
		return values;
