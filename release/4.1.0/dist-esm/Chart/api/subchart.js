 ...
 * @version 4.1.0
*/
import { extend, parseDate } from "../../module/util/object.js";
import { $COMMON } from "../../config/classes.js";
//#region src/Chart/api/subchart.ts
/**
...
const subchart = function(domainValue) {
	...
	if (state.isCanvasMode) {
		if (config.subchart_show) {
			...
			} else domain = state.domain ?? x.orgDomain();
		}
		return domain;
	}
	if (config.subchart_show) {
		domain = domainValue;
		if (Array.isArray(domain)) {
			if (axis.isTimeSeries()) domain = domain.map((x) => parseDate.bind($$)(x));
			if ($$.withinRange(domain, $$.getZoomDomain("subX", true), $$.getZoomDomain("subX"))) {
