 ...
 * @version 4.1.0
*/
import { isDefined, isObject } from "../../module/util/type-checks.js";
import { extend, getMinMax, parseDate } from "../../module/util/object.js";
import { zoomIdentity, zoomTransform } from "d3-zoom";
//#region src/Chart/api/zoom.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
*  chart.zoom();
*/
const zoom$1 = function(domainValue) {
	const $$ = this.internal;
	...
	if (config.zoom_enabled) {
		...
		if (Array.isArray(domain)) {
			...
			if ($$.withinRange(domain, $$.getZoomDomain("zoom", true), $$.getZoomDomain("zoom"))) {
				state.domain = domain.slice();
				domain = $$.getZoomDomainValue(domain);
				$$.api.tooltip.hide();
				if (config.subchart_show) {
					if (state.isCanvasMode) $$.setCanvasSubchartDomain?.(domain, true, false);
					else {
...
extend(zoom$1, {
	...
	*/
	enable(enabled) {
		...
		if (!$$.zoom) $$.initZoom();
		else if (enabled === false) $$.bindZoomEvent(false);
		if (enabled !== false) {
	...
	*/
	max(max) {
		const { config, org: { xDomain } } = this.internal;
	...
	*/
	unzoom() {
		...
		if (zoom || state.isCanvasMode && config.subchart_show && state.domain) {
			config.subchart_show ? state.isCanvasMode ? $$.clearCanvasSubchartDomain?.(true, false) : $$.brush.getSelection().call($$.brush.move, null) : $$.zoom.updateTransformScale(zoomIdentity);
			$$.updateZoom(true);
			zoomResetBtn?.style("display", "none");
			if (target?.node() && zoomTransform(target.node()) !== zoomIdentity) $$.zoom.transform(target, zoomIdentity);
			state.domain = void 0;
		}
	}
};
