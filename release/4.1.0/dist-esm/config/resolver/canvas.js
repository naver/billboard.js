...
import { callFn, extend, parseDate, tplProcess } from "../../module/util/object.js";
import { getBoundingRect } from "../../module/util/dom.js";
import { $FOCUS, $LEGEND } from "../classes.js";
import { KEY } from "../../module/Cache.js";
import ChartInternal from "../../ChartInternal/ChartInternal.js";
import { getRenderDataPoint } from "../../ChartInternal/shape/core/geometry.js";
import { getMainCoordFromSubchartCoord } from "../../ChartInternal/internals/subchart.util.js";
import { hasCanvasDrawableValue, isCanvasBarType, isCanvasBubbleType, isCanvasCandlestickType, isCanvasPointType, isCanvasScatterType, isCanvasTargetSupported, isCanvasTreemapType } from "../../canvas/util.js";
import CanvasAxisRenderer from "../../canvas/CanvasAxisRenderer.js";
import { $CANVAS } from "../../canvas/classes.js";
import CanvasEngine from "../../canvas/CanvasEngine.js";
import CanvasRenderer from "../../canvas/CanvasRenderer.js";
import CanvasTheme from "../../canvas/CanvasTheme.js";
import HitDetector from "../../canvas/HitDetector.js";
import { select } from "d3-selection";
//#region src/config/resolver/canvas.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
function warn(message) {
	win.console?.warn?.(`[billboard.js] ${message}`);
}
/**
...
function getCanvasLegendPointPattern($$, id) {
	...
	const pattern = $$.getValidPointPattern?.() || (Array.isArray(config.point_pattern) && config.point_pattern.length ? config.point_pattern : [config.point_type]);
	return pattern[Math.max(0, targetIds.indexOf(id)) % pattern.length] || "circle";
}
/**
...
function getCanvasLegendPointIcon($$, id) {
	...
	if (/^rect(angle)?$/i.test(pattern)) return `<svg ${svgAttrs}><rect x="1" y="1" width="6" height="6" ${paintAttrs}></rect></svg>`;
	if (!pattern || /^circle$/i.test(pattern)) return `<svg ${svgAttrs}><circle cx="4" cy="4" r="3" ${paintAttrs}></circle></svg>`;
...
function setCanvasHtmlLegendItem($$, item) {
	...
	item.attr("class", function(id) {
		const next = `${select(this).attr("class") || ""} ${$$.generateClass($LEGEND.legendItem, id)}`;
...
function setCanvasHtmlLegendFocus($$, id) {
	...
	legend?.selectAll(`.${$LEGEND.legendItem}`).classed($FOCUS.legendItemFocused, (d) => targetIds.indexOf(d) >= 0).style("opacity", function(d) {
		return targetIds.indexOf(d) >= 0 ? null : $$.opacityForUnfocusedLegend.call($$, select(this));
	});
}
/**
...
function revertCanvasHtmlLegendFocus($$) {
	const { legend } = $$.$el;
	legend?.selectAll(`.${$LEGEND.legendItem}`).classed($FOCUS.legendItemFocused, false).style("opacity", null);
}
/**
...
function setCanvasLegendTargetFocus($$, id) {
	const { state } = $$;
	const focusedIds = ($$.mapToTargetIds?.([id]) || [id]).filter($$.isTargetToShow, $$);
	const focusedSet = new Set(focusedIds);
	const defocusedIds = ($$.mapToTargetIds?.() || []).filter((targetId) => !focusedSet.has(targetId) && $$.isTargetToShow(targetId));
...
function revertCanvasLegendTargetFocus($$) {
	...
	changed && $$.renderCanvasFrame?.(void 0, null, false);
}
/**
...
function updateCanvasLegendTouchMove($$, event) {
	...
	if (touch) start.moved = start.moved || Math.abs(touch.clientX - start.x) > CANVAS_LEGEND_TOUCH_TAP_THRESHOLD || Math.abs(touch.clientY - start.y) > CANVAS_LEGEND_TOUCH_TAP_THRESHOLD;
}
/**
...
function bindCanvasHtmlLegendInteractions($$, item) {
	...
	const eventType = typeof interaction === "object" && interaction?.dblclick ? "dblclick" : "click";
	...
	item.on(eventType, hasClickInteraction ? function(event, id) {
		if (isTouch && event.type === "click" && isDuplicateCanvasLegendTouchClick($$, id)) return;
...
function getCanvasEventClientPoint(event) {
	...
	return touchEvent.changedTouches?.[0] || touchEvent.touches?.[0] || ("clientX" in event ? event : null);
}
/**
...
function getCanvasEventDatum($$, event, shapeOnly = false) {
	...
	return point ? shapeOnly ? $$.hitDetector.findNearestShape?.(point[0], point[1]) : $$.hitDetector.findNearest(point[0], point[1]) : null;
}
/**
...
function isCanvasXIndexHoverArea($$, point) {
	...
	if ($$.isMultipleX?.() || state.hasTreemap) return false;
...
function getCanvasHoverDatumFromPoint($$, point) {
	return $$.hitDetector.findNearest(point[0], point[1]) || (isCanvasXIndexHoverArea($$, point) ? $$.hitDetector.findNearestIndexByCoord?.(point[0], point[1]) : null);
}
/**
...
function getCanvasTooltipData($$, d) {
	...
	const sameXData = config.tooltip_grouped && !state.hasTreemap && !pointBased ? $$.filterByX?.(targetsToShow, d.x) : null;
	return (state.hasTreemap ? [d] : pointBased ? [d] : config.tooltip_grouped ? sameXData?.length ? sameXData : targetsToShow.map((target) => target.values[d.index]).filter(Boolean) : [d]).map((v) => $$.addName?.(v) || v);
}
/**
...
function getCanvasFocusData($$, d, selectedData) {
	return $$.isMultipleX?.() && !isCanvasPointBasedInteraction($$, d) ? [$$.addName?.(d) || d] : selectedData;
}
/**
...
function getCanvasSubchartRect($$) {
	const { config, state } = $$;
	if (!config.subchart_show || !state.hasAxis || state.width2 <= 0 || state.height2 <= 0) return null;
	return {
...
function getCanvasMainPointFromSubchart($$, point) {
	const { config, state } = $$;
	const rect = getCanvasSubchartRect($$);
	if (config.subchart_brush_enabled !== false || !rect || !isCanvasSubchartPoint($$, point)) return null;
	const mainCoord = getMainCoordFromSubchartCoord($$, config.axis_rotated ? point[1] - rect.y : point[0] - rect.x);
...
function getCanvasSubchartDomainFromCoords($$, start, end) {
	...
	return [$$.scale.subX.invert(coordStart), $$.scale.subX.invert(coordEnd)];
}
/**
...
function getCanvasSubchartBrushSelection($$) {
	...
	if (!domain?.length || !scale.subX) return null;
	const rect = getCanvasSubchartRect($$);
	const p0 = scale.subX(domain[0]);
	const p1 = scale.subX(domain[1]);
	if (!rect || !Number.isFinite(p0) || !Number.isFinite(p1)) return null;
	const [min, max] = getCanvasSubchartBrushExtent($$);
	return [Math.max(min, Math.min(max, Math.min(p0, p1))), Math.max(min, Math.min(max, Math.max(p0, p1)))];
}
/**
...
function getCanvasSubchartBrushMode($$, coord) {
	...
	if (Math.abs(coord - start) <= CANVAS_SUBCHART_HANDLE_SIZE) return "resize-start";
	if (Math.abs(coord - end) <= CANVAS_SUBCHART_HANDLE_SIZE) return "resize-end";
...
function clampCanvasSubchartSelection($$, start, end) {
	const [min, max] = getCanvasSubchartBrushExtent($$);
	return [Math.max(min, Math.min(max, start)), Math.max(min, Math.min(max, end))];
}
/**
...
function getCanvasSubchartCursor($$, mode) {
	if (/^resize/.test(mode)) return $$.config.axis_rotated ? "ns-resize" : "ew-resize";
...
const canvasInternal = {
	...
	*/
	prepareCanvasConfig() {
		...
		if (state.orgConfig?.transition && Object.prototype.hasOwnProperty.call(state.orgConfig.transition, "duration") && state.orgConfig.transition.duration) warn("canvas mode: transition.duration is ignored; canvas redraws synchronously.");
		config.transition_duration = 0;
		if (!isSupportedCanvasXType(config.axis_x_type)) config.axis_x_type = "indexed";
		if (!isSupportedCanvasYType(config.axis_y_type)) config.axis_y_type = "indexed";
		if (!isSupportedCanvasYType(config.axis_y2_type)) config.axis_y2_type = "indexed";
	...
	*/
	applyCanvasSubchartDomain() {
		...
		if (!config.subchart_show || !state.hasAxis || !scale.subX) return;
		if (!state.rendered && !state.domain && config.subchart_init_range) state.domain = normalizeCanvasSubchartDomain($$, config.subchart_init_range);
		const domain = normalizeCanvasSubchartDomain($$, state.domain);
		if (!domain || !$$.withinRange(domain, $$.getZoomDomain("subX", true), $$.getZoomDomain("subX"))) return;
		state.domain = domain;
	...
	*/
	setCanvasSubchartDomain(domain, redraw = true, callCallback = false) {
		...
		if (!config.subchart_show || !nextDomain || !$$.withinRange(nextDomain, $$.getZoomDomain("subX", true), $$.getZoomDomain("subX"))) return;
	...
	*/
	clearCanvasSubchartDomain(redraw = true, callCallback = false) {
		...
		if (!config.subchart_show) return;
		state.domain = void 0;
		scale.zoom = null;
	...
	*/
	flushCanvasFlow() {
		const { state } = this;
		if (!state.canvasFlowFinish) return;
	...
	*/
	animateCanvasFlow(flow) {
		...
		if (!requestFrame || !duration || !length || !orgDataCount || getCanvasFlowValueCount($$) > CANVAS_FLOW_ANIMATION_MAX_VALUES) return false;
		...
		if (!isCanvasFlowDomain(startDomain, isLog)) return false;
		...
		const render = (timestamp) => {
			...
			$$.renderCanvasFrame(void 0, null, false);
			if (ratio < 1) state.canvasFlowFrame = requestFrame(render);
			else finish();
		};
		startOrgDomain && (org.xDomain = startOrgDomain);
		scale.x.domain(startDomain);
	...
	*/
	initCanvas() {
		...
		if (win.getComputedStyle(container).position === "static") $el.chart.style("position", "relative");
		...
		$el.canvas = select($$.canvasEngine.canvas);
	...
	*/
	warnUnsupportedCanvasOptions() {
		const { config } = this;
		[
			[!isSupportedCanvasXType(config.axis_x_type), "axis.x.type other than indexed/category/log/timeseries"],
			[!isSupportedCanvasYType(config.axis_y_type), "axis.y.type other than indexed/log/timeseries"],
			[!isSupportedCanvasYType(config.axis_y2_type), "axis.y2.type other than indexed/log/timeseries"],
	...
	*/
	bindCanvasEvents() {
		...
		canvas.addEventListener("mousedown", (event) => {
			if ($$.onCanvasSubchartBrushStart?.(event)) return;
	...
	updateHtmlLegend() {
		...
		if (!config.legend_show || state.hasTreemap) {
			if (state.hasTreemap) {
				$el.legend?.remove();
				$el.legend = null;
			} else $el.legend?.style("visibility", "hidden");
		...
		if (!$el.legend) $el.legend = select(chart).append("div").classed($LEGEND.legend, true).classed($CANVAS.legend, true);
		...
		item.select(`.${$LEGEND.legendItemTile}`).classed($CANVAS.legendItemTileCircle, !config.legend_usePoint && config.legend_item_tile_type === "circle").style("background-color", (id) => config.legend_usePoint ? null : $$.color(id)).html((id) => config.legend_usePoint ? getCanvasLegendPointIcon($$, id) : "");
		item.select(`.${$CANVAS.legendItemTitle}`).text((id) => getLegendText($$, id));
		if (config.legend_tooltip) item.attr("title", (id) => getLegendText($$, id));
	...
	*/
	updateHtmlLegendSize(targetIds) {
		...
		if (useTemplateLegend) $el.legend.selectAll(`.${$LEGEND.legendItem}`).each(function(id) {
			...
			const measured = targetIds.map((id, index) => {
				...
				const width = hidden ? 0 : (rect?.width || fallbackWidth) + dimension.tileWidth + (isLast && !isRightOrInset ? 0 : dimension.padding.right) + config.legend_padding;
				const height = hidden ? 0 : (rect?.height || fallbackHeight) + dimension.padding.top;
		...
		if (useTemplateLegend) {
			...
			itemSizes.forEach((width) => {
				if (rowWidth && rowWidth + rowGap + width > availableWidth) {
					rows++;
					rowWidth = width;
				} else rowWidth += (rowWidth ? rowGap : 0) + width;
			});
			state.legendStep = Math.max(0, rows - 1);
		} else state.legendStep = legendStep;
		state.legendItemWidth = maxWidth;
		state.legendItemHeight = Math.max(12, maxHeight);
		if (!useTemplateLegend) $el.legend.selectAll(`button.${$LEGEND.legendItem}`).each(function(id) {
	...
	*/
	getCanvasBottomLegendHeight() {
		const $$ = this;
		const { config, state, $el } = $$;
		if (!config.legend_show || state.isLegendRight || state.isLegendInset || state.hasTreemap || !$el.legend || !state.legendItemHeight) return 0;
	...
	*/
	positionHtmlLegend() {
		...
		if (state.isLegendRight) legend.style("top", `${state.margin3.top}px`).style("right", "0").style("bottom", `${state.margin3.bottom}px`).style("width", `${Math.max(0, $$.getLegendWidth())}px`);
			...
			legend.style(isTop ? "top" : "bottom", `${config.legend_inset_y}px`).style(isLeft ? "left" : "right", `${config.legend_inset_x}px`).style("width", `${Math.max(0, state.legendItemWidth * (state.legendStep + 1) + 20)}px`);
		}
	},
	/**
	...
	*/
	getCanvasSelectedData(targetId) {
		...
		const data = [];
		if (!$$.config.data_selection_enabled || !selected?.size) return data;
	...
	*/
	setCanvasSelection(isSelection = false, ids, indices, resetOther) {
		...
		if (singleSelection) {
			...
			if (!selectionGrouped) {
				if (targetIds) targetIds = targetIds.slice(0, 1);
				else {
	...
	*/
	toggleCanvasSelection(dataRows) {
		...
		dataRows.filter((d) => isCanvasSelectableData($$, d)).forEach((d) => {
			const key = getCanvasDataKey(d);
			const isSelected = selected.has(key);
			if (!config.data_selection_multiple) eachCanvasSelectableData($$, (selectedData) => {
				const selectedKey = getCanvasDataKey(selectedData);
				if ((config.data_selection_grouped ? selectedData.id === d.id : selectedKey !== key) && selected.has(selectedKey)) {
					selected.delete(selectedKey);
					callFn(config.data_onunselected, $$.api, selectedData, $$.canvasEngine.canvas);
					changed = true;
				}
			});
	...
	*/
	canStartCanvasSelectionDrag(event) {
		const { config } = this;
		const mouse = event;
		return Boolean(config.interaction_enabled && config.data_selection_enabled && config.data_selection_draggable && config.data_selection_multiple && !config.zoom_enabled && (mouse.button === void 0 || mouse.button === 0));
	},
	/**
	...
	*/
	getCanvasSelectionDragData(rect) {
		...
		if (!config.data_selection_grouped) return data;
	...
	*/
	startCanvasSelectionDrag(event) {
		...
		const { state } = $$;
		const point = getCanvasEventPoint($$, event);
		if (!point || !$$.canStartCanvasSelectionDrag(event)) return false;
	...
	*/
	updateCanvasSelectionDrag(event) {
		...
		const point = getCanvasEventPoint($$, event);
		if (!state.canvasSelectionDragging || !start || !point) return false;
		const [sx, sy] = start;
		const [mx, my] = point;
		...
		if (moved) {
			const { included, rows } = getCanvasSelectionDragDelta($$, $$.getCanvasSelectionDragData(rect));
	...
	*/
	endCanvasSelectionDrag(event) {
		const $$ = this;
		const { state } = $$;
		if (!state.canvasSelectionDragging) return false;
		$$.updateCanvasSelectionDrag(event);
	...
	*/
	dispatchCanvasDataOver(d, canvas) {
		...
		const { config, state } = $$;
		const key = getCanvasDataKey(d);
		if (state.canvasDataHoverKey === key) return;
		$$.dispatchCanvasDataOut(canvas);
	...
	*/
	dispatchCanvasDataClick(event, markInputClick = false) {
		...
		if (!d || isDuplicateCanvasInputClick($$, d)) return d;
	...
	*/
	updateCanvasSubchartBrush(event) {
		...
		if (config.subchart_brush_enabled === false || !state.canvasSubchartBrushDragging || start === null || coord === null || !mode) return false;
		const delta = coord - start;
		if (Math.abs(delta) > CANVAS_SUBCHART_CLICK_TOLERANCE) state.canvasSubchartBrushMoved = true;
		...
		if (mode === "move" && origin) {
			...
			selection = [Math.max(extentStart, Math.min(extentEnd - size, nextStart)), Math.min(extentEnd, Math.max(extentStart + size, nextEnd))];
		} else if (mode === "resize-start" && origin) selection = clampCanvasSubchartSelection($$, coord, origin[1]);
		else if (mode === "resize-end" && origin) selection = clampCanvasSubchartSelection($$, origin[0], coord);
		else selection = clampCanvasSubchartSelection($$, start, coord);
	...
	*/
	startCanvasSubchartBrush(event) {
		const $$ = this;
		const { state } = $$;
		if ($$.config.subchart_brush_enabled === false) return false;
		const coord = getCanvasSubchartBrushCoord($$, event);
	...
	*/
	endCanvasSubchartBrush(event) {
		const $$ = this;
		const { state } = $$;
		if (!state.canvasSubchartBrushDragging) return false;
		$$.updateCanvasSubchartBrush(event);
		...
		if (shouldClear) $$.clearCanvasSubchartDomain?.(true, true);
		else if (shouldCallback) $$.config.subchart_onbrush.bind($$.api)(state.domain ?? $$.scale.x.orgDomain());
	...
	*/
	onCanvasSubchartBrushStart(event) {
		const $$ = this;
		if (!$$.startCanvasSubchartBrush(event)) return false;
		const move = (event) => {
			$$.updateCanvasSubchartBrush(event);
		};
		const end = (event) => {
	...
	*/
	onCanvasSelectionDragStart(event) {
		const $$ = this;
		const { state } = $$;
		if (!$$.startCanvasSelectionDrag(event)) return;
		const move = (event) => {
			$$.updateCanvasSelectionDrag(event);
		};
		const end = (event) => {
	...
	*/
	onCanvasPointerDown(event) {
		if (!shouldHandleCanvasPointerEvent(this, event)) return;
		const $$ = this;
		const canvas = $$.$el.canvas.node();
		if (!$$.startCanvasSubchartBrush(event)) return;
	...
	*/
	onCanvasPointerMove(event) {
		if (!shouldHandleCanvasPointerEvent(this, event)) return;
		if (this.updateCanvasSubchartBrush(event)) return;
	...
	*/
	onCanvasPointerOut(event) {
		if (!shouldHandleCanvasPointerEvent(this, event)) return;
		if (this.state.canvasSubchartBrushDragging) return;
		this.state.event = event;
	...
	*/
	onCanvasTouchStart(event) {
		...
		this.state.event = event;
		if (this.startCanvasSubchartBrush(event)) return;
		if (this.startCanvasSelectionDrag(event)) return;
		this.config.onover?.bind(this.api)(event);
		this.onCanvasMouseMove(event);
	},
	/**
	...
	*/
	onCanvasTouchMove(event) {
		if (event.touches.length > 1) return;
		if (this.updateCanvasSubchartBrush(event)) return;
		if (this.updateCanvasSelectionDrag(event)) return;
		this.onCanvasMouseMove(event);
	},
	/**
	...
	*/
	onCanvasMouseMove(event) {
		...
		if (state.canvasSelectionDragging) return;
	...
	*/
	onCanvasMouseOut() {
		...
		} else if (this.config.axis_tooltip) this.clearCanvasFocus();
		this.hideTooltip?.();
	},
	/**
	...
	*/
	showCanvasLinkedTooltip(index) {
		...
		const { $el, state } = $$;
		const canvas = $el.canvas?.node();
		const d = $$.filterTargetsToShow($$.data.targets).map((target) => target.values[index]).find(Boolean);
		if (!canvas || !d) {
			$$.hideTooltip?.();
			return false;
		}
		...
		const eventPoint = [state.margin.left + point.x, state.margin.top + point.y];
		state.event = {
	...
	*/
	clearCanvasFocus() {
		...
		if ($$.state.canvasFocusMainRedraw) {
			$$.state.canvasFocusMainRedraw = false;
			$$.renderCanvasFrame(void 0, null, false);
		} else $$.canvasEngine?.clearOverlay();
	},
	/**
