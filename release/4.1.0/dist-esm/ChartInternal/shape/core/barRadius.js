...
//#region src/ChartInternal/shape/core/barRadius.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
function getArcFlagTokenIndexes(path) {
	...
	while (token = RE_PATH_TOKEN.exec(path)) if (token[1]) {
		...
		if (inArc) {
			...
			if (pos === 2 || pos === 3 || pos === 4) flags.add(numIndex);
			argIndex++;
		}
	}
	return flags;
...
function getBarPathInterpolator(a, b) {
	...
	let bm;
	const append = (text) => {
		if (segments[segIndex]) segments[segIndex] += text;
		else segments[++segIndex] = text;
	};
	...
	while ((am = reA.exec(a)) && (bm = reB.exec(b))) {
		numIndex++;
		if (bm.index > bIndex) append(b.slice(bIndex, bm.index));
		if (am[0] === bm[0]) append(bm[0]);
		else {
			const from = +am[0];
			const to = +bm[0];
	...
	if (bIndex < b.length) append(b.slice(bIndex));
...
function getStackingBarRadiusSet($$) {
	...
	if (!config.data_groups.length) return set;
	...
	for (const group of config.data_groups) {
		...
		for (const [index, id] of lastPosByIndex) set.add(`${id}:${index}`);
		for (const [index, id] of lastNegByIndex) set.add(`${id}:${index}`);
	}
	return set;
}
/**
...
function getBarRadiusClipPath(init, pos, isRotated, isNegative) {
	let clipPath = "";
	if (isRotated) {
		if (isNegative && init[0] < pos) clipPath = `0 ${pos - init[0]}px 0 0`;
		else if (!isNegative && init[0] > pos) clipPath = `0 0 0 ${init[0] - pos}px`;
	} else if (isNegative && init[1] > pos) clipPath = `${init[1] - pos}px 0 0 0`;
	else if (!isNegative && init[1] < pos) clipPath = `0 0 ${pos - init[1]}px 0`;
...
function getBarRadiusInfo($$, d, points, getRadius, stackingRadiusSet, isStackingRadiusData) {
	...
	const init = [points[0][indexX], points[0][indexY]];
	let radius = 0;
	if (getRadius) {
...
//#endregion
export { getBarPathInterpolator, getBarRadiusInfo, getBarRadiusResolver, getStackingBarRadiusSet };
