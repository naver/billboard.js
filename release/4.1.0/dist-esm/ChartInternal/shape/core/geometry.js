 ...
 * @version 4.1.0
*/
//#region src/ChartInternal/shape/core/geometry.ts
/**
...
function getRenderDataPoint($$, d) {
	const value = $$.isGrouped?.(d.id) && $$.circleY ? $$.circleY(d, d.index) : $$.getYScaleById(d.id)($$.getBaseValue(d));
...
function getTreemapLabelText($$, data, width, height) {
	...
	const text = typeof format === "function" ? format.bind($$.api)(value, ratio, id, {
		width,
		height
	}) : `${id}\n${(ratio * 100).toFixed(2)}%`;
...
//#endregion
export { getCandlestickGeometry, getRenderDataPoint, getRenderPoint, getRenderRect, getShapePoint, getTreemapLabelText, getTreemapNodeRect };
