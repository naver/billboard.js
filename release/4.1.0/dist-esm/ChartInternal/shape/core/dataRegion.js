 ...
 * @version 4.1.0
*/
import { isUndefined, isValue } from "../../../module/util/type-checks.js";
import { parseDate } from "../../../module/util/object.js";
//#region src/ChartInternal/shape/core/dataRegion.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
function getDataRegionStyle(x, regions) {
	for (let i = 0, region; region = regions[i]; i++) if (region.start < x && x <= region.end) return region.style;
	return false;
}
/**
...
function getDataRegionDash(style) {
	const dash = String(style?.dasharray || DEFAULT_DASHARRAY).split(/[\s,]+/).map(Number).filter(Number.isFinite);
