 ...
 * @version 4.1.0
*/
import { isFunction, isNumber, isObject } from "../../../module/util/type-checks.js";
import { getMinMax } from "../../../module/util/object.js";
import { KEY } from "../../../module/Cache.js";
//#region src/ChartInternal/shape/core/bubble.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
var bubble_default = {
	...
	*/
	getBubbleR(d) {
		...
		if (isFunction(maxR)) maxR = maxR.bind($$.api)(d);
		else if (!isNumber(maxR)) maxR = $$.getBaseLength() / ($$.getMaxDataCount() * 2) + 12;
		const cacheKey = KEY.bubbleMaxValue;
		let max = $$.cache.get(cacheKey);
		if (max === null) $$.cache.add(cacheKey, max = getMinMax("max", $$.getMinMaxData().max.map((d) => $$.isBubbleZType(d) ? $$.getBubbleZData(d.value, "y") : isObject(d.value) ? d.value.mid : d.value)));
