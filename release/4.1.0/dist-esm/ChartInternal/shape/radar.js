 ...
 * @version 4.1.0
*/
import { isDefined, isEmpty, isNumber, isUndefined } from "../../module/util/type-checks.js";
import { getMinMax, getRange, toArray } from "../../module/util/object.js";
import { getBoundingRect, getPathBox, setTextValue } from "../../module/util/dom.js";
import { $AXIS, $COMMON, $LEVEL, $RADAR, $SHAPE, $TEXT } from "../../config/classes.js";
import { KEY } from "../../module/Cache.js";
import { select } from "d3-selection";
//#region src/ChartInternal/shape/radar.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
function _getPosition(isClockwise, type, edge, pos, range, ratio) {
	...
	const r = 2 * Math.PI;
	return range * (1 - ratio * (type === "x" ? Math.sin : Math.cos)(index * r / edge));
}
...
var radar_default = {
	...
	getRadarSize() {
		const { config, state: { arcWidth, arcHeight } } = this;
		const size = (Math.min(arcWidth, arcHeight) - (config.axis_x_categories.length < 4 ? -20 : 10)) / 2;
		return [size, size];
	},
	updateTargetsForRadar(targets) {
		const $$ = this;
		const { config } = $$;
		if (isEmpty(config.axis_x_categories)) config.axis_x_categories = getRange(0, getMinMax("max", targets.map((v) => v.values.length)));
		$$.generateRadarPoints();
	},
	...
	*/
	...
	updateRadarLevel() {
		...
		const points = levelData.map((v) => {
			...
			return getRange(0, edge).map((i) => $$.getRadarPosition(["x", "y"], i, range, 1).join(",")).join(" ");
		});
		...
		if (showText) {
			if (radarLevels.select("text").empty()) radarLevels.append("text").attr("dx", "-.5em").attr("dy", "-.7em").style("text-anchor", "end").text(() => levelTextFormat(0));
			levelEnter.append("text").attr("dx", "-.5em").style("text-anchor", "end").text((d) => levelTextFormat(state.current.dataMax / levelData.length * (d + 1)));
		}
		levelEnter.merge(level).attr("transform", (d) => `translate(${width - levelRatio[d]}, ${height - levelRatio[d]})`).selectAll("polygon").attr("points", (d) => points[d]);
		if (showText) radarLevels.selectAll("text").attr("x", (d) => isUndefined(d) ? width : points[d].split(",")[0]).attr("y", (d) => isUndefined(d) ? height : 0);
	},
	updateRadarAxes() {
		const $$ = this;
		...
		if (config.radar_axis_line_show) axis.select("line").attr("x1", width).attr("y1", height).attr("x2", (d, i) => $$.getRadarPosition("x", i)).attr("y2", (d, i) => $$.getRadarPosition("y", i));
		if (config.radar_axis_text_show) {
			...
			axis.select("text").style("text-anchor", "middle").attr("dy", ".5em").call((selection) => {
				selection.each(function(d) {
					setTextValue(select(this), String(d), [-.6, 1.2]);
				});
			}).datum((d, i) => ({ index: i })).attr("transform", function(d) {
				if (isUndefined(this.width)) this.width = getBoundingRect(this, true).width / 2;
				let posX = $$.getRadarPosition("x", d.index, void 0, 1);
				let posY = Math.round($$.getRadarPosition("y", d.index, void 0, 1));
				if (posX > width) posX += this.width + x;
				else if (Math.round(posX) < width) posX -= this.width + x;
				if (posY > height) {
					if (posY / 2 === height && this.firstChild.tagName === "tspan") this.firstChild.setAttribute("dy", "0em");
					posY += y;
				} else if (posY < height) posY -= y;
				return `translate(${posX} ${posY})`;
			});
			if (!textWidth) {
				const widths = [radar.axes, radar.levels].map((v) => getPathBox(v.node()).width);
				if (widths.every((v) => v > 0)) $$.cache.add(cacheKeyTextWidth, widths[0] - widths[1]);
			}
		}
	},
	bindRadarEvent() {
		...
		if (!isMouse) svg.on("touchstart", hide, { passive: true });
	},
	updateRadarShape() {
		const $$ = this;
