...
import { $COMMON, $FUNNEL } from "../../config/classes.js";
import { select } from "d3-selection";
import { curveLinear, line } from "d3-shape";
//#region src/ChartInternal/shape/funnel.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
function _getSplineClipPath() {
	...
	const lineGen = line().x((d) => d[0]).y((d) => d[1]).curve(curveLinear);
	...
	if (edge1.length > 1) {
		...
		if (lastIdx > 0) edge2.splice(lastIdx, 0, [edge2[lastIdx - a][0], edge2[lastIdx - b][1]]);
	}
	...
	generateGetFunnelPoints() {
		...
		targets.each((d) => {
			...
			points[d.id] = isRotated ? [
				[segmentStart, crossCenter],
				[segmentEnd, crossCenter],
				[center, crossCenter]
			] : [
				[crossCenter, segmentStart],
				[crossCenter, segmentEnd],
				[crossCenter, center]
			];
		});
...
//#endregion
export { funnel_default as default };
