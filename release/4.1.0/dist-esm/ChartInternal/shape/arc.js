...
import { isDefined, isFunction, isNumber, isObject, isUndefined } from "../../module/util/type-checks.js";
import { callFn, endall, tplProcess } from "../../module/util/object.js";
import { setTextValue } from "../../module/util/dom.js";
import { $ARC, $COMMON, $FOCUS, $GAUGE } from "../../config/classes.js";
import { meetsLabelThreshold, updateTextImage, updateTextImagePos } from "../internals/text.util.js";
import { isLabelWithLine, redrawArcLabelLines } from "../internals/text.arc.js";
import { select } from "d3-selection";
import { arc, pie } from "d3-shape";
import { interpolate } from "d3-interpolate";
//#region src/ChartInternal/shape/arc.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
function _calculateLabelRatio($$, d, outerRadius, distance) {
	...
	if (ratio) ratio = isFunction(ratio) ? ratio.bind($$.api)(d, outerRadius, distance) : ratio;
	else {
		const LABEL_MIN_SIZE = 36;
		const LABEL_RATIO_THRESHOLD = .375;
		const LABEL_RATIO_BASE = 1.175;
		const LABEL_RATIO_LARGE = .8;
		const labelSpaceRatio = LABEL_MIN_SIZE / outerRadius;
		ratio = outerRadius && distance ? (labelSpaceRatio > LABEL_RATIO_THRESHOLD ? LABEL_RATIO_BASE - labelSpaceRatio : LABEL_RATIO_LARGE) * outerRadius / distance : 0;
	}
	return ratio;
}
/**
...
function _getRadiusFn(expandRate = 0) {
	...
	const expandWidth = expandRate ? Math.min(state.radiusExpanded * expandRate - state.radius, singleArcWidth * .8 - (1 - expandRate) * 100) : 0;
	return {
		/**
		...
		*/
		outer(d) {
			...
			let radius;
			if (hasMultiArcGauge) radius = state.radius - singleArcWidth * d.index + expandWidth;
			else if ($$.hasType("polar") && !expandRate) radius = $$.getPolarOuterRadius(d, outerRadius);
			else {
				radius = outerRadius;
				if (expandRate) {
		...
		*/
		corner(d, outerRadius) {
			...
			let corner;
			if (ratio) corner = ratio * outerRadius;
			else corner = isNumber(cornerRadius) ? cornerRadius : cornerRadius.call($$.api, id, value, outerRadius);
			return corner;
		}
	};
}
...
var arc_default = {
	initPie() {
		...
		const padAngle = (padding ? padding * .01 : config[`${dataType}_padAngle`]) || 0;
		$$.pie = pie().startAngle(startingAngle).endAngle(startingAngle + 2 * Math.PI).padAngle(padAngle).value((d) => d.values?.reduce((a, b) => a + b.value, 0) ?? d).sort($$.getSortCompareFn.bind($$)(true));
	},
	updateRadius() {
		const $$ = this;
		const { config, state } = $$;
		const padding = config[`${config.data_type}_padding`];
		const w = config.gauge_width || config.donut_width;
		const gaugeArcWidth = $$.getTargetsToShow().length * config.gauge_arcs_minWidth;
		const LABEL_RADIUS_RATIO = .85;
	...
	*/
	getRadius(d) {
		...
		if (isObject(outerRadius) && data && data.id in outerRadius) outerRadius = outerRadius[data.id];
		else if (!isNumber(outerRadius)) outerRadius = $$.state.radius;
		return {
			innerRadius,
			outerRadius
		};
	...
	getArcLength() {
		...
		if (arcLengthInPercent < -360) len = -2;
		else if (arcLengthInPercent > 360) len = 2;
		return len * Math.PI;
	},
	getStartingAngle() {
		const $$ = this;
		...
		if (!isFullCircle && startAngle <= defaultStartAngle) startAngle = defaultStartAngle;
		else if (!isFullCircle && startAngle >= defaultEndAngle) startAngle = defaultEndAngle;
		else if (startAngle > Math.PI || startAngle < -1 * Math.PI) startAngle = Math.PI;
		return startAngle;
	},
	/**
	* Update angle data
	...
	*/
	updateAngle(dValue, forRange = false) {
		...
		if (isSingleArcGauge) {
			...
			const gEnd = radius * (($$.getTotalDataSum(state.rendered) - gMin) / (gMax - gMin));
			pie = pie.startAngle(gStart).endAngle(gEnd + gStart);
		}
		if (forRange === false) {
			let layout;
			if (isSingleArcGauge) layout = pie($$.filterTargetsToShow());
			else {
	...
	getSvgArc() {
		...
		const arc$1 = arc().innerRadius(inner).outerRadius(outer);
		const newArc = function(d, withoutUpdate) {
			let path = "M 0 0";
			if (d.value || d.data) {
				const data = withoutUpdate ? d : $$.updateAngle(d) ?? null;
				if (data) path = arc$1.cornerRadius(corner(data, outer(data)))(data);
			}
			return path;
		};
	...
	*/
	getSvgArcExpanded(rate = 1) {
		...
		const arc$2 = arc().innerRadius(inner).outerRadius(outer);
		return (d) => {
			const updated = $$.updateAngle(d);
			const outerR = outer(updated);
			let cornerR = 0;
			if (updated) cornerR = corner(updated, outerR);
			return updated ? arc$2.cornerRadius(cornerR)(updated) || "M 0 0" : "M 0 0";
		};
	},
	...
	*/
	redrawArcRangeText() {
		...
		if (values?.length) {
			...
			if (isPercent) values = values.map((v) => totalSum / 100 * v);
			...
			if ((!state.rendered || state.rendered && !fixed) && totalSum > 0) rangeText.attr("transform", function(d, i) {
	...
	*/
	transformForArcLabel(textNode, d, forRange = false) {
		...
		let pos;
		let ratio = 1;
		if (forRange || $$.hasMultiArcGauge()) pos = _calculateRangeOrGaugePosition($$, d, updated, forRange);
	...
	textForArcLabel(selection) {
		...
		if ($$.shouldShowArcLabel()) selection.style("fill", $$.updateTextColor.bind($$)).attr("filter", (d) => $$.updateTextBGColor.bind($$)(d, $$.config.data_labels_backgroundColors)).each(function(d) {
	...
	expandArc(targetIds) {
		...
		$el.svg.selectAll($$.selectorTargets(newTargetIds, `.${$ARC.chartArc}`)).each(function(d) {
			...
			select(this).selectAll("path").transition().duration(expandDuration).attrTween("d", _getAttrTweenFn($$.svgArcExpanded.bind($$))).transition().duration(expandDuration * 2).attrTween("d", _getAttrTweenFn(svgArcExpandedSub.bind($$)));
		});
	},
	...
	*/
	getExpandConfig(id, key) {
		...
		let type;
		if ($$.isDonutType(id)) type = "donut";
		else if ($$.isGaugeType(id)) type = "gauge";
		else if ($$.isPieType(id)) type = "pie";
	...
	getArcLabelConfig(name = "format") {
		...
		if (name === "format") return isFunction(fn) ? fn.bind($$.api) : fn;
		else return fn;
	},
	...
	*/
	setArcTitle(str) {
		...
		if (title) {
			...
			if (text.empty()) text = $$.$el.arcs.append("text").attr("class", className).style("text-anchor", "middle");
			hasGauge && text.attr("dy", "-0.3em");
			setTextValue(text, title, hasGauge ? void 0 : [-.6, 1.35], true);
		}
	},
	/**
	...
	*/
	getArcTitleWithNeedleValue() {
		...
		if (title && $$.config.arc_needle_show && /{=[A-Z_]+}/.test(title)) {
			...
			if (!isNumber(value)) value = config.arc_needle_value;
			return tplProcess(title, { NEEDLE_VALUE: ~~value });
		}
		return false;
	},
	redrawArc(duration, durationForExit, withTransform) {
		...
		mainArc = mainArc.enter().append("path").attr("class", $$.getClass("arc", true)).style("fill", (d) => $$.color(d.data)).style("cursor", (d) => isSelectable?.bind?.($$.api)(d) ? "pointer" : null).style("opacity", "0").each(function(d) {
		...
		mainArc.attr("transform", (d) => !$$.isGaugeType(d.data) && withTransform ? "scale(0)" : "").style("opacity", function(d) {
			...
			const interpolate$2 = interpolate(this._current, updated);
			this._current = interpolate$2(0);
			return function(t) {
				const interpolated = interpolate$2(t);
		...
		if (needle.empty()) {
			...
			*/
			$el.needle.updateHelper = (v, updateConfig = false) => {
				if ($el.needle.style("display") !== "none") $$.$T($el.needle).style("transform", `rotate(${$$.getNeedleAngle(v)}deg)`).call(endall, () => {
	...
	*/
	getNeedleAngle(v) {
		...
		let radian;
		if (!isNumber(value)) value = hasGauge && $$.data.targets.length === 1 ? total : 0;
		...
		} else radian = total ? arcLength * (value / total) : 0;
	...
	redrawBackgroundArcs() {
		...
		if (hasMultiArcGauge) {
			...
			backgroundArc.enter().append("path").attr("class", (d, i) => `${$ARC.chartArcsBackground} ${$ARC.chartArcsBackground}-${i}`).merge(backgroundArc).style("fill", config.gauge_background || null).attr("d", ({ id }) => {
				if (showEmptyTextLabel || state.hiddenTargetIds.has(id)) return "M 0 0";
				const d = {
		...
		} else backgroundArc.attr("d", showEmptyTextLabel ? "M 0 0" : () => {
			return $$.getArc({
				data: [{ value: config.gauge_max }],
				startAngle,
				endAngle
	...
	bindArcEvent(arc) {
		...
		if (isMouse) arc.on("mouseover", function(event, d) {
			if (state.transiting) return;
			state.event = event;
			const arcData = _getArcData(d);
			const id = arcData?.id || void 0;
			selectArc(this, arcData, id);
			$$.setOverOut(true, arcData);
		}).on("mouseout", (event, d) => {
			if (state.transiting || !config.interaction_onout) return;
			state.event = event;
			const arcData = _getArcData(d);
			unselectArc();
		...
		if (isTouch && $$.hasArcType() && !$$.radars) {
			const getEventArc = (event) => {
				...
				return select(doc.elementFromPoint(clientX, clientY));
			};
			$$.$el.svg.on("touchstart touchmove", function(event) {
				if (state.transiting) return;
				state.event = event;
				const datum = getEventArc(event).datum();
	...
	redrawArcText(duration) {
		...
		if (!(hasGauge && $$.data.targets.length === 1 && config.gauge_title)) {
			...
			}).transition().duration(duration).style("opacity", (d) => $$.isTargetToShow(d.data.id) && $$.isArcType(d.data) ? null : "0");
	...
	*/
	getArcElementByIdOrIndex(value) {
		...
		return arcs?.selectAll(`.${$COMMON.target} path`).filter(filterFn);
	}
};
//#endregion
