...
//#region src/ChartInternal/internals/size.axis.ts
var size_axis_default = {
	/**
	...
	*/
	...
	getAxisWidthByAxisId(id, withoutRecompute) {
		...
		if ($$.axis) {
			...
			const gap = width === 0 ? .5 : 0;
			return width + ($$.config.padding?.mode === "fit" ? position.isInner ? 10 + gap : 10 : position.isInner ? 20 + gap : 40);
		} else return 40;
	},
	getHorizontalAxisHeight(id) {
		...
		if (id === "y2" && !config.axis_y2_show) return isFitPadding ? 0 : rotatedPadding.top;
	...
	getAxisTickRotate(id) {
		...
		if (id === "x") {
			...
			if (config.axis_x_tick_fit && allowedXAxisTypes) {
				...
				let tickCount = 0;
				if (xTickCount) tickCount = xTickCount > currentXTicksLength ? currentXTicksLength : xTickCount;
				else if (currentXTicksLength) tickCount = currentXTicksLength;
			...
			if (($el.svg || state.isCanvasMode) && config.axis_x_tick_autorotate && config.axis_x_tick_fit && !config.axis_x_tick_multiline && !config.axis_x_tick_culling && allowedXAxisTypes) rotate = $$.needToRotateXAxisTickTexts() ? config.axis_x_tick_rotate : 0;
		}
		return rotate;
	},
	/**
	...
	*/
	needToRotateXAxisTickTexts() {
		...
		const xAxisLength = current.width - (isLegendRight && legendItemWidth) - $$.getCurrentPaddingByDirection("left") - $$.getCurrentPaddingByDirection("right");
		const tickCountWithPadding = axis.x.tickCount + axis.x.padding.left + axis.x.padding.right;
		const { width } = $$.axis.getMaxTickSize("x");
		return width > (tickCountWithPadding ? xAxisLength / tickCountWithPadding : 0);
	}
};
//#endregion
export { size_axis_default as default };
