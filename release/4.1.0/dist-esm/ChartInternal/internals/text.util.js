...
import { parseShorthand, toMap, tplProcess } from "../../module/util/object.js";
import { getBBox, getBoundingRect } from "../../module/util/dom.js";
import { $TEXT } from "../../config/classes.js";
import { select } from "d3-selection";
//#region src/ChartInternal/internals/text.util.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
function getTextPos(d, type) {
	...
	return (isFunction(position) ? position.bind(this.api)(type, value, id, index, this.$el.text) : (id in position ? position[id] : position)[type]) ?? 0;
}
/**
* Update text border
...
function updateTextBorder(text, pos, rectClass) {
	...
	if (borderRect.empty() || borderRect.node()?.tagName !== "rect" || !borderRect.attr("class")?.includes(rectClass)) {
		borderRect = select(text.parentNode).insert("rect", () => text).attr("class", `${$TEXT.textBorderRect} ${rectClass}`).attr("width", textRect.width + (applyStyle ? borderPadding.left + borderPadding.right : 0)).attr("height", textRect.height + (applyStyle ? borderPadding.top + borderPadding.bottom : 0));
		if (applyStyle) borderRect.style("fill", fill).style("stroke", stroke).style("stroke-width", `${strokeWidth}px`).attr("rx", radius).attr("ry", radius);
	}
	$T(borderRect).attr("x", pos.x - (applyStyle ? borderPadding.left : 0) - (isRotated ? 0 : textRect.width / 2)).attr("y", pos.y - (applyStyle ? borderPadding.top : 0) - textRect.height / 4 * 3.2);
}
/**
...
function meetsLabelThreshold(ratio = 0, type) {
	const { config } = this;
	return ratio >= (config[`${type}_label_threshold`] || 0);
}
/**
* Update text image
* @private
...
function updateTextImage() {
	...
	if ($$.state.arcWidth ? $$.getArcLabelConfig("image") : config.data_labels.image) text.filter(function() {
		const prev = this.previousElementSibling;
		if (prev) return prev.tagName !== "image" || !prev.classList.contains($TEXT.textLabelImage);
		return true;
	}).each(function(d) {
		const image = getDataLabelImgUrl.call($$, d);
		if (!image) return;
		const { url, width, height, pos } = image;
		if (url) select(this.parentNode)?.insert("image", `${this.getAttribute("class")?.replace(/(?:^(.)|\s)/g, ".$1") ?? "text"}`).style("opacity", "0").attr("href", (d) => tplProcess(url, { ID: "id" in d ? d.id : d.data.id })).attr("class", $TEXT.textLabelImage).style("pointer-events", "none").attr("width", width).attr("height", height).attr("transform", pos ? `translate(${pos.x ?? 0} ${pos.y ?? 0})` : null);
	});
}
/**
...
function updateTextImagePos(textNode, pos, textRect) {
	...
	const isShown = (textNode) => {
		return (arcWidth ? textNode.textContent : textNode.style.opacity !== "0" && textNode.style.fillOpacity !== "0") && this.previousElementSibling?.tagName !== "image";
	};
	if (!image.empty() && image.node()?.tagName === "image") {
		...
		if (isRotated) pos.x += w;
		else {
			if (hasTreemap) {
				x = -w;
				y = -(h * 2 + textDimension.height);
			}
			if (!($$.hasType("pie") || $$.hasType("polar"))) pos.y += h;
		}
