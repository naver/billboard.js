 ...
 * @version 4.1.0
*/
import { asHalfPixel } from "../../module/util/type-checks.js";
import { $ARC, $AXIS } from "../../config/classes.js";
//#region src/ChartInternal/internals/transform.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
var transform_default = {
	getTranslate(target, index = 0) {
		...
		if (target === "main") {
			...
			x = state.arcWidth / 2;
			y = state.arcHeight / 2;
			if (config.arc_rangeText_values?.length) y += 5 + ($$.hasType("gauge") && config.title_text ? 10 : 0);
	...
	transformMain(withTransition, transitions) {
		const $$ = this;
		const { $el: { main }, $T } = $$;
		const xAxis = transitions?.axisX ? transitions.axisX : $T(main.select(`.${$AXIS.axisX}`), withTransition);
		const yAxis = transitions?.axisY ? transitions.axisY : $T(main.select(`.${$AXIS.axisY}`), withTransition);
		const y2Axis = transitions?.axisY2 ? transitions.axisY2 : $T(main.select(`.${$AXIS.axisY2}`), withTransition);
