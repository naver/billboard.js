...
import { getBBox, getBoundingRect, getElementPos, setTextValue } from "../../module/util/dom.js";
import { $TEXT } from "../../config/classes.js";
//#region src/ChartInternal/internals/title.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
function _getTextXPos(pos = "left", width) {
	...
	if (pos.includes("center")) return isNum ? width / 2 : "middle";
	if (pos.includes("right")) return isNum ? width : "end";
...
function _getCanvasTitleHeight($$) {
	const { config, $el } = $$;
	const font = $$.canvasTheme?.style?.title?.font || $$.canvasTheme?.style?.label?.font || "14px sans-serif";
...
var title_default = {
	...
	*/
	getTitlePadding() {
		...
		if (state.isCanvasMode && config.title_text) return paddingTop + $$.getCanvasTitleHeight() + paddingBottom;
		if (!title?.node()) return paddingTop + paddingBottom;
