...
import drag_default from "../interactions/drag.js";
import { select } from "d3-selection";
//#region src/ChartInternal/internals/selection.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
var selection_default = {
	...
	*/
	getToggle(that, d) {
		const $$ = this;
		return that.nodeName === "path" ? $$.togglePath : $$.isStepType(d) ? () => {} : $$.togglePoint;
	},
	/**
	...
	toggleShape(that, d, i) {
		...
		if (config.data_selection_enabled && config.data_selection_isselectable.bind($$.api)(d)) {
			...
			if (!config.data_selection_multiple) {
				...
				main.selectAll(selector).selectAll(focusOnly ? `.${$SELECT.selectedCircle}` : `.${$SHAPE.shape}.${$SELECT.SELECTED}`).classed($SELECT.SELECTED, false).each(function(d) {
