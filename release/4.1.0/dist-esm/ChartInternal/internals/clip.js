 ...
 * @version 4.1.0
*/
//#region src/ChartInternal/internals/clip.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
var clip_default = {
	...
	*/
	setXAxisClipPath(node) {
		const { config, state: { margin, width, height } } = this;
	...
	*/
	...
	setXAxisTickClipWidth() {
		...
		} else maxTickSize.x.clipPath = null;
	},
	setXAxisTickTextClipPathWidth() {
		const { state: { clip, current }, $el: { svg } } = this;
		if (svg) svg.select(`#${clip.idXAxisTickTexts} rect`).attr("width", current.maxTickSize.x.clipPath).attr("height", 30);
	}
};
//#endregion
