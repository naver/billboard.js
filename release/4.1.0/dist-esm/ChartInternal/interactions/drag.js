 ...
 * @version 4.1.0
*/
import { getPathBox, scheduleRAFUpdate } from "../../module/util/dom.js";
import { $BAR, $CIRCLE, $COMMON, $DRAG, $SELECT, $SHAPE } from "../../config/classes.js";
import { select } from "d3-selection";
//#region src/ChartInternal/interactions/drag.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
var drag_default = {
	...
	*/
	drag(mouse) {
		...
		if ($$.hasArcType() || !config.data_selection_enabled || config.zoom_enabled && !$$.zoom.altDomain || !config.data_selection_multiple) return;
		...
		const executeDrag = () => {
			...
			main.selectAll(`.${$SHAPE.shapes}`).selectAll(`.${$SHAPE.shape}`).filter((d) => isSelectable?.bind($$.api)(d)).each(function(d, i) {
...
//#endregion
export { drag_default as default };
