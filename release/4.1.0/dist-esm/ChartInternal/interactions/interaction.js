 ...
 * @version 4.1.0
*/
import { isNumber, isObject } from "../../module/util/type-checks.js";
import { emulateEvent, getBoundingRect, getPointer, getTransformCTM, hasViewBox } from "../../module/util/dom.js";
import { $ARC, $AXIS, $COMMON, $SHAPE } from "../../config/classes.js";
import { KEY } from "../../module/Cache.js";
import { select } from "d3-selection";
import { drag } from "d3-drag";
//#region src/ChartInternal/interactions/interaction.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
var interaction_default = {
	...
	setOverOut(isOver, d) {
		...
		if (isArcishData || d !== -1) {
			...
			if (isArcishData) {
				...
				const shapesAtIndex = main.selectAll(`.${$SHAPE.shape}-${d}`).filter(function(d) {
					return $$.isWithinShape(this, d);
				});
				const shape = shapesAtIndex.filter(function() {
					return last.every((v) => v !== this);
				});
				if (!isOver || shapesAtIndex.empty() || last.length === shape.size() && shape.nodes().every((v, i) => v !== last[i])) while (last.length) {
				...
				if (isOver) hasRadar && $$.isPointFocusOnly() ? $$.showCircleFocus($$.getAllValuesOnIndex(d, true)) : $$.setExpand(d, null, true);
	...
	*/
	dispatchEvent(type, index, mouse) {
		...
		let element = ((hasFunnel || hasTreemap) && eventReceiver.rect || hasRadar && radar.axes.select(`.${$AXIS.axis}-${index} text`) || eventRect || $$.getArcElementByIdOrIndex?.(index))?.node?.();
		if (element) {
			...
			if (hasFunnel || hasTreemap) element = (funnel ?? treemap).node();
	...
	*/
	unbindZoomEvent() {
		const { $el: { canvas, eventRect, svg, zoomResetBtn } } = this;
