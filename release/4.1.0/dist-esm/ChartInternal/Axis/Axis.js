 ...
 * @version 4.1.0
*/
import { isArray, isEmpty, isFunction, isNumber, isObjectType, isString, isValue, notEmpty } from "../../module/util/type-checks.js";
import { capitalize, mergeObj, parseDate, sortValue } from "../../module/util/object.js";
...
//#region src/ChartInternal/Axis/Axis.ts
/**
...
* @private
*/
function getAxisTickOption(config, id, key) {
...
function _sampleTickNodes(nodes) {
	...
	if (longestNode && !sampled.includes(longestNode)) sampled.push(longestNode);
	const mid = nodes[Math.floor(nodes.length / 2)];
	if (!sampled.includes(mid)) sampled.push(mid);
	return sampled;
}
...
function _getTickLineWidth(tickNodes) {
	...
	const strokeWidth = line?.ownerDocument?.defaultView?.getComputedStyle ? parseFloat(line.ownerDocument.defaultView.getComputedStyle(line).strokeWidth) : parseFloat(line?.getAttribute?.("stroke-width"));
...
function _hasOverlappedTickLineIntervals(axis, tickValues, tickLineWidth) {
	...
	if (!scale || tickValues.length < 2) return false;
...
function _sampleTickValues(values, format) {
	if (values.length <= MAX_TICK_MEASURE_VALUES) return values;
	const sampled = /* @__PURE__ */ new Map();
	const add = (index) => {
		if (index >= 0 && index < values.length) sampled.set(index, values[index]);
	};
	add(0);
	add(values.length - 1);
	add(Math.floor(values.length / 2));
	const step = Math.max(1, Math.floor(values.length / 47));
	let maxLength = -1;
	let maxIndex = 0;
...
function _getTickValuesCacheValue(values) {
	if (isFunction(values)) return _getCacheReferenceId(values);
	if (!Array.isArray(values) || values.length <= MAX_TICK_MEASURE_VALUES) return values;
	return {
...
function _setTickWidthFallback(ticks, width) {
	if (isNumber(width)) Object.defineProperty(ticks, TICK_WIDTH_FALLBACK, {
		configurable: true,
		value: width,
		writable: true
	});
	else delete ticks[TICK_WIDTH_FALLBACK];
}
/**
...
function _getCacheReferenceId(value) {
	if (!value || !/^(function|object)$/.test(typeof value)) return `${typeof value}:${String(value)}`;
...
function _stringifyCacheValue(value) {
	...
	else if (Array.isArray(value)) return `[${value.map((v) => _stringifyCacheValue(v)).join(",")}]`;
	else if (value && typeof value === "object") return `{${Object.keys(value).sort().map((key) => `${key}:${_stringifyCacheValue(value[key])}`).join(",")}}`;
	else if (typeof value === "function") return _getCacheReferenceId(value);
...
function _restoreMaxTickSize(target, source) {
	...
	if (target.ticks && source.ticks) _restoreTickWidths(target.ticks, source.ticks);
	return target;
}
var Axis_default = { getAxisInstance: function() {
...
var Axis = class {
	...
	getAxisType(id = "x") {
		let type = "linear";
		if (this.isTimeSeries(id)) type = this.owner.config.axis_x_localtime ? "time" : "utc";
		else if (this.isLog(id)) type = "log";
		return type;
	}
	/**
	* Get extent value
	...
	*/
	getExtent() {
		...
		if (extent) {
			if (isFunction(extent)) extent = extent.bind($$.api)($$.getXDomain($$.data.targets), scale.subX);
	...
	init() {
		...
		target.forEach((v) => {
			...
			axis[v] = main.append("g").attr("class", classAxis).attr("clip-path", () => {
				let res = null;
				if (v === "x") res = clip.pathXAxis;
				else if (v === "y") res = clip.pathYAxis;
				return res;
	...
	*/
	setOrient() {
		const { axis_rotated: isRotated, axis_y_inner: yInner, axis_y2_inner: y2Inner } = this.owner.config;
		this.orient = {
	...
	*/
	generateAxes(id) {
		...
		const isRotated = config.axis_rotated;
		let orient;
		if (id === "x") orient = isRotated ? "left" : "bottom";
		else if (id === "y") orient = isRotated ? "bottom" : "left";
		else if (id === "y2") orient = isRotated ? "top" : "right";
		if (axesConfig.length) axesConfig.forEach((v) => {
			const tick = v.tick || {};
			...
			const axis = new AxisRenderer({
				...
				isSubAxes: true,
				owner: $$
			}).scale(scale).orient(orient).tickFormat(isFunction(tick.format) ? tick.format.bind($$.api) : ((x) => x));
	...
	updateAxes() {
		...
		Object.keys(this.axesList).forEach((id) => {
			...
			this.axesList[id].forEach((v, i) => {
				...
				if (!range.every((v, i) => v === axisRange[i])) v.scale().range(range);
	...
	*/
	setAxis(id, scale, outerTick, noTransition) {
		const $$ = this.owner;
		const type = getBaseAxisId(id);
		if (!isSubAxis(id)) this.tick[type] = this.getTickValues(type);
		this[id] = this.getAxis(id, scale, outerTick, id === "x" && ($$.scale.zoom || $$.config.subchart_show || $$.state.resizing) ? true : noTransition);
	}
	getAxis(id, scale, outerTick, noTransition, noTickTextRotate) {
		...
		let tickFormat;
		if (isX) tickFormat = id === "subX" ? $$.format.subXAxisTick : $$.format.xAxisTick;
		else {
			...
			if (isFunction(fn)) tickFormat = fn.bind($$.api);
		}
		...
		axis.tickFormat(tickFormat || !isX && $$.isStackNormalized() && $$.hasAxisGroupedData(type) && ((x) => `${x}%`));
		if (isCategory) {
	...
	updateXAxisTickValues(targets, axis) {
		...
		const generateValues = (countOption = config.axis_x_tick_count) => {
			...
			if (fit) {
				...
				if (this.isCategorized() && count > values.length) count = values.length;
				values = this.generateTickValues(values, count, this.isTimeSeries());
			}
			return values;
		};
		const values = generateValues();
		if (axis) axis.tickValues(values);
		else if (this.x) {
			const subTickValues = this.getTickValues("x", true);
			const subTickCount = getAxisTickOption(config, "subX", "count");
			const subValues = subTickValues ?? (subTickCount !== config.axis_x_tick_count ? generateValues(subTickCount) : values);
	...
	getXAxisTickFormat(forSubchart) {
		...
		let currFormat;
		if (tickFormat) {
			if (isFunction(tickFormat)) currFormat = tickFormat.bind($$.api);
			else if (isTimeSeries) currFormat = (date) => date ? format.axisTime(tickFormat)(date) : "";
		} else currFormat = isTimeSeries ? format.defaultAxisTime : isCategorized ? $$.categoryName : (v) => v < 0 ? v.toFixed(0) : v;
		return isFunction(currFormat) ? (v) => currFormat.apply($$, isCategorized ? [v, $$.categoryName(v)] : [v]) : currFormat;
	}
	getTickValues(id, isSub = false) {
		const $$ = this.owner;
		const tickValues = isSub ? getAxisTickOption($$.config, `sub${capitalize(id)}`, "values") : $$.config[`axis_${id}_tick_values`];
		const values = isFunction(tickValues) ? tickValues.call($$.api) : tickValues;
		const axis = $$[`${id}Axis`];
		return isSub ? values ?? void 0 : values || (axis ? axis.tickValues() : void 0);
	}
	...
	setLabelText(id, text) {
		const { config } = this.owner;
		const option = this.getLabelOptionByAxisId(id);
		if (isString(option)) config[`axis_${id}_label`] = text;
		else if (option) option.text = text;
	}
	...
	xForAxisLabel(id) {
		...
		if (this.isHorizontal($$, id !== "x")) x = position.isLeft ? 0 : position.isCenter ? width / 2 : width;
		else if (position.isBottom) x = -height;
		return x;
	}
	textAnchorForAxisLabel(id) {
		...
		if (this.isHorizontal($$, id !== "x")) anchor = position.isLeft ? "start" : position.isCenter ? "middle" : "end";
		else if (position.isBottom) anchor = "start";
		return anchor;
	}
	dxForAxisLabel(id) {
		const $$ = this.owner;
		...
		if (this.isHorizontal($$, id !== "x")) dx = position.isLeft ? "0.5em" : position.isRight ? "-0.5em" : "0";
		else if (position.isTop) dx = "-0.5em";
		return dx;
	}
	dyForAxisLabel(id) {
		const $$ = this.owner;
		...
		if (id === "x") {
			...
			if (isRotated) dy = isInner ? "1.2em" : -25 - maxTickWidth;
			else if (isInner) dy = "-0.5em";
			else if (xHeight) dy = xHeight - 10;
			else if (tickRotate) dy = tickRotate - 10;
			else dy = "3em";
		} else {
			dy = {
				y: [
			...
			if (isRotated) {
				if (isInner) dy = dy[0];
				else if (tickRotate) dy = tickRotate * (id === "y2" ? -1 : 1) - dy[1];
				else dy = dy[2];
			} else dy = isInner ? dy[3] : (dy[4] + (config[`axis_${id}_inner`] ? 0 : maxTickWidth + dy[4])) * (id === "y" ? -1 : 1);
		}
		return dy;
	}
	...
	*/
	getMaxTickSize(id, withoutRecompute) {
		...
		if (cached && cached.generation === state.redrawGeneration) return currentTickMax;
		...
		let fingerprint;
		if (resizing || withoutRecompute || !config[`${configPrefix}_show`] || currentTickMax.width > 0 && $$.filterTargetsToShow().length === 0) return currentTickMax;
		if ((svg || config.render_mode === "canvas") && $$.scale[id]?.copy) {
			...
			if (isDomainSame || isCurrentMaxTickDomainSame) return currentTickMax;
			else currentTickMax.domain = domain;
			if (!isYAxis) _clearTickWidths(currentTickMax.ticks);
			...
			if (!config[`${configPrefix}_tick_values`] && tickCount) axis.tickValues(this.generateTickValues(domain, tickCount, isYAxis ? this.isTimeSeriesY() : this.isTimeSeries()));
			...
			if (sizeFor1Char) textSelection.each(function(d, i) {
				...
				if (!isYAxis) currentTickMax.ticks[i] = width;
			});
			else {
				const textNodes = [];
				...
				(textNodes.length <= 5 ? textNodes : _sampleTickNodes(textNodes)).map((node) => getBoundingRect(node, true)).forEach((dim) => {
					max.width = Math.max(max.width, dim.width);
					max.height = Math.max(max.height, dim.height);
				});
				if (!isYAxis) for (let i = 0; i < textNodes.length; i++) currentTickMax.ticks[i] = max.width;
			}
			if (!isYAxis && hasLargeTickValues) _compactTickWidths(currentTickMax.ticks, measuredTickCount, max.width);
			dummy.remove();
		}
		Object.keys(max).forEach((key) => {
			if (max[key] > 0) currentTickMax[key] = max[key];
		});
		$$.cache.add(cacheKey, {
			fingerprint,
			generation: state.redrawGeneration,
	...
	getXAxisTickMaxOverflow(xAxisTickRotate, widthWithoutCurrentPaddingLeft) {
		...
		const fallbackTickTextWidth = _getTickWidthFallback(tickTextWidths) ?? state.current.maxTickSize.x.width;
		...
		for (let i = 0; i < tickCount; i++) {
			...
			const ticksBeforeTickText = tickIndex - (isTimeSeries ? 1 : .5) + left;
			if (ticksBeforeTickText <= 0) continue;
	...
	*/
	updateLabels(withTransition) {
		const { config, $el: { main }, $T } = this.owner;
		const isRotated = config.axis_rotated;
		[
			"x",
			"y",
			...
			if (text) {
				...
				if (axisLabel.empty()) axisLabel = main.select(`g.${$AXIS[selector]}`).insert("text", ":first-child").attr("class", classLabel).attr("transform", ["rotate(-90)", null][id === "x" ? +!isRotated : +isRotated]).style("text-anchor", () => this.textAnchorForAxisLabel(id));
	...
	*/
	...
	generateTickValues(values, tickCount, forTimeSeries) {
		...
		if (tickCount) {
			...
			if (targetCount === 1) tickValues = [values[0]];
			else if (targetCount === 2) tickValues = [values[0], values[values.length - 1]];
	...
	redraw(transitions, isHidden, isInit) {
		const { config, state, $el } = this.owner;
		...
		[
			...
			if (axis && $axis) {
				if (!isInit && !config.transition_duration) axis.config.withoutTransition = true;
	...
	*/
	syncAxisDomains(targetsToShow, wth, flow) {
		...
		let xDomainForZoom;
		if (!hasZoom && this.isCategorized() && targetsToShow.length === 0 && $el.axis.x) scale.x.domain([0, $el.axis.x.selectAll(".tick").size()]);
		...
		if (config.zoom_rescale && !flow) xDomainForZoom = scale.x.orgDomain();
		["y", "y2"].forEach((key) => {
		...
		if (wth.Y) {
			const updateSubDomain = () => {
				...
				["y", "y2"].forEach((key) => {
					...
					if (!axisScale || !axis || tickValues || !tickCount) return;
					const domain = axisScale.domain();
					axis.tickValues(this.generateTickValues(domain, domain.every((v) => v === 0) ? 1 : tickCount, this.isTimeSeriesY()));
				});
			};
	...
	*/
	redrawAxis(targetsToShow, wth, transitions, flow, isInit) {
		...
		if ((wth.UpdateXDomain || wth.UpdateXAxis || wth.Y) && targetsToShow.length) this.setCulling();
	}
	/**
	* Set manual culling
	* @private
	*/
	setCulling() {
		const { config, state: { clip, current }, $el } = this.owner;
		[
			"subX",
			"x",
			"y",
			...
			if (axis && toCull) {
				...
				const tickValues = sortValue(tickNodes.data(), !getAxisTickOption(config, type, "culling_reverse"));
				const tickSize = tickValues.length;
				const cullingMax = getAxisTickOption(config, type, "culling_max");
				const cullTickLine = !getAxisTickOption(config, type, "culling_lines") || _hasOverlappedTickLineIntervals(this[type], tickValues, _getTickLineWidth(tickNodes));
				...
				if (tickSize) {
					...
					for (let i = 0; i < tickValues.length; i++) tickIndexMap.set(tickValues[i], i);
					tickNodes.each(function(d) {
						const node = cullTickLine ? this : this.querySelector("text");
						if (node) node.style.display = (tickIndexMap.get(d) ?? 0) % intervalForCulling ? "none" : null;
					});
				} else tickNodes.style("display", null);
				if (type === "x") {
	...
	setAxisTooltip() {
		...
		[
			...
			if (isString(bgColor) || bgColor[v]) {
				axisTooltip[v] = axis[v]?.append("text").classed($AXIS[`axis${v.toUpperCase()}Tooltip`], true).attr("filter", $$.updateTextBGColor({ id: v }, bgColor));
				if (isRotated) axisTooltip[v]?.attr(v === "x" ? "x" : "y", v === "y" ? "1.15em" : v === "x" ? "-0.3em" : "-0.4em").attr(`d${v === "x" ? "y" : "x"}`, v === "x" ? "0.4em" : "-1.3em").style("text-anchor", v === "x" ? "end" : null);
				else axisTooltip[v]?.attr(v === "x" ? "y" : "x", v === "x" ? "1.15em" : `${v === "y" ? "-" : ""}0.4em`).attr(`d${v === "x" ? "x" : "y"}`, v === "x" ? "-1em" : "0.3em").style("text-anchor", v === "y" ? "end" : null);
			}
		});
	}
};
