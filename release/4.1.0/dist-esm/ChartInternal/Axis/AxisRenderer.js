 ...
 * @version 4.1.0
*/
import { isArray, isFunction, isNumber, isString } from "../../module/util/type-checks.js";
...
//#region src/ChartInternal/Axis/AxisRenderer.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
* @ignore
...
var AxisRenderer = class {
	...
	canReuseTickNodesOnResize(tickNodes, ticks, isLeftRight, tickShow) {
		if (tickShow.tick || !tickShow.text || !this.canReuseTickTextOnResize(isLeftRight) || tickNodes.size() !== ticks.length) return false;
	...
	*/
	create(g) {
		...
		const tickTextPos = !isSubAxes && type && /^(x|y|y2)$/.test(type) ? params.config[`axis_${type}_tick_text_position`] : {
			x: 0,
			y: 0
		};
		const prefix = getAxisOptionPrefix(id);
		const axisShow = params.config[`${prefix}_show`];
		const tickShow = isSubAxes ? {
			tick: true,
			text: true
		} : {
		...
		g.each(function() {
			...
			if (tickShow.tick || tickShow.text) {
				...
				if (ctx.canReuseTickNodesOnResize(tick, ticks, isLeftRight || params.config.axis_rotated, tickShow)) tickTransform(tick, scale1);
				else {
					...
					if (tickShow.text && !reuseTickText) {
						...
						if (isFunction(evalTextSize)) {
							...
							if (this.classList.contains($COMMON.dummy)) this.sizeFor1Char = sizeFor1Char;
						}
						if (!sizeFor1Char || sizeFor1Char.w === 0 || sizeFor1Char.h === 0) sizeFor1Char = ctx.helper.getSizeFor1Char(orient, tickText, !!evalTextSize);
						let tspan = tickText.selectAll("tspan").data((d, index) => {
							let split;
							if (params.tickMultiline) split = splitTickText(d, scale1, ticks, isLeftRight, sizeFor1Char.w);
							else {
						...
						tspan.attr("x", isTopBottom ? 0 : tickLength * sign).attr("dx", (() => {
							let dx = 0;
							if (/(top|bottom)/.test(orient) && rotate) dx = 8 * Math.sin(Math.PI * (rotate / 180)) * (orient === "top" ? -1 : 1);
							...
							if (orient !== "top") {
								dy = sizeFor1Char.h;
								if (i === 0) dy = isLeftRight ? -((counts[d.index] - 1) * (sizeFor1Char.h / 2) - 3) : tickTextPos.y === 0 ? defValue : 0;
							}
					...
					if (scale1.bandwidth) {
						...
						scale0 = (d) => x(d) + dx;
						scale1 = scale0;
					} else if (scale0.bandwidth) scale0 = scale1;
					else tickTransform(tickExit, scale1);
	...
	*/
		...
		switch (orient) {
			case "bottom":
				...
				textUpdate.attr("x", 0).attr("y", yForText(rotate)).style("text-anchor", (d, i, { length }) => {
					if (!isRotated && i === 0 && (inner === true || inner.first)) return "start";
					else if (!isRotated && i === length - 1 && (inner === true || inner.last)) return "end";
	...
	splitTickText(d, scale, ticks, isLeftRight, charWidth) {
		...
		if (!tickWidth || tickWidth <= 0) tickWidth = isLeftRight ? 95 : params.isCategory ? (params.isInverted ? scale(ticks[0]) - scale(ticks[1]) : scale(ticks[1]) - scale(ticks[0])) - 12 : 110;
		function split(splitted, text) {
			let subtext;
			let spaceIndex;
			let textWidth;
			for (let i = 1; i < text.length; i++) {
				if (text.charAt(i) === " ") spaceIndex = i;
				subtext = text.substr(0, i + 1);
				textWidth = charWidth * subtext.length;
				if (tickWidth < textWidth) return split(splitted.concat(text.substr(0, spaceIndex || i)), text.slice(spaceIndex ? spaceIndex + 1 : i));
			}
	...
	scale(x) {
		if (!arguments.length) return this.helper.scale;
		this.helper.scale = x;
		return this;
	}
	orient(x) {
		if (!arguments.length) return this.config.orient;
	...
	tickFormat(format) {
		const { config } = this;
		if (!arguments.length) return config.tickFormat;
		config.tickFormat = format;
		return this;
	}
	tickCentered(isCentered) {
		const { config } = this;
		if (!arguments.length) return config.tickCentered;
	...
	*/
	tickInterval(size) {
		...
		if (this.params.isCategory) {
			...
			const scale = this.params.owner.scale.zoom ?? this.helper.scale;
			interval = (this.g ? this.g.select("path.domain").node().getTotalLength() - outerTickSize * 2 : Math.abs(scale.range()[1] - scale.range()[0])) / (size || this.g?.selectAll("line").size() || 1);
			if (tickValues) for (let i = 0; i < tickValues.length - 1; i++) {
				const intervalByValue = scale(tickValues[i + 1]) - scale(tickValues[i]);
				if (intervalByValue && intervalByValue < interval) interval = intervalByValue;
			}
		}
	...
	ticks(...args) {
		const { config } = this;
		if (!args.length) return config.tickArguments;
		config.tickArguments = toArray(args);
		return this;
	}
	tickCulling(culling) {
		const { config } = this;
		if (!arguments.length) return config.tickCulling;
		config.tickCulling = culling;
		return this;
	}
	tickValues(x) {
		const { config } = this;
		if (isFunction(x)) config.tickValues = () => x(this.helper.scale.domain());
		else {
