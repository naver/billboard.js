...
import { isArray, isBoolean, isDefined, isFunction, isNumber, isObject, isObjectType, isString, isUndefined, isValue, notEmpty } from "../../module/util/type-checks.js";
import { findIndex, getUnique, hasValue, mergeArray, parseDate, sortValue } from "../../module/util/object.js";
import { getScrollPosition, getTransformCTM, hasViewBox } from "../../module/util/dom.js";
import { $BAR, $CANDLESTICK, $COMMON } from "../../config/classes.js";
import { KEY } from "../../module/Cache.js";
import { select } from "d3-selection";
//#region src/ChartInternal/data/data.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
var data_default = {
	...
	*/
	isStackNormalizedPerGroup() {
		const { config } = this;
		return !!(isObjectType(config.data_stack_normalize) && config.data_stack_normalize?.perGroup && config.data_groups.length);
	},
	/**
	...
	*/
	hasAxisGroupedData(axisId) {
		const $$ = this;
		const { axis } = $$;
		return $$.data.targets.filter((t) => axis.getId(t.id) === axisId).map((t) => t.id).some((id) => $$.isGrouped(id));
	},
	getXKey(id) {
		const { config } = this;
	...
	getXValuesOfXKey(key, targets) {
		...
		let xValues;
		ids.forEach((id) => {
			if ($$.getXKey(id) === key) xValues = $$.data.xs[id];
		});
		return xValues;
	},
	/**
	...
	*/
	...
	getValueOnIndex(values, index) {
		if (values[index]?.index === index) return values[index];
	...
	generateTargetX(rawX, id, index) {
		...
		} else if (axis?.isCustomX() && !axis?.isCategorized()) x = isValue(rawX) ? +rawX : $$.getXValue(id, index);
		return x;
	},
	updateXs(values) {
		if (values.length) this.axis.xs = values.map((v) => v.x);
	},
	getPrevX(i) {
	...
	*/
	getBaseValue(data) {
		...
		let { value } = data;
		if (value && hasAxis) {
			if ($$.isAreaRangeType(data)) value = $$.getRangedData(data, "mid");
			else if ($$.isBubbleZType(data)) value = $$.getBubbleZData(value, "y");
		}
		return value;
	},
	/**
	...
	getMinMaxData() {
		...
		if (!minMaxData) {
			...
			data.forEach((v) => {
				...
				if (minData.length) for (let i = 0; i < minData.length; i++) min.push(minData[i]);
				if (maxData.length) for (let i = 0; i < maxData.length; i++) max.push(maxData[i]);
			});
	...
	getTotalPerIndex(targetId) {
		...
		if (($$.config.data_groups.length || $$.isStackNormalized()) && !sum) {
			...
			if ($$.isStackNormalizedPerGroup() && targetId) {
				...
				if (group) targets = targets.filter((t) => group.indexOf(t.id) >= 0);
				else return null;
			}
			targets.forEach((row) => {
	...
	*/
	getVisibleTotalPerIndex(targetId) {
		...
		if (!cached || cached.generation !== state.dataGeneration) cached = cache.add(KEY.visibleTotalPerIndex, {
			generation: state.dataGeneration,
			values: /* @__PURE__ */ new Map()
		});
		if (cached.values.has(targetId)) return cached.values.get(targetId);
		...
		if (hiddenTargetIds.size) {
			...
			if (targetId) {
				const group = config.data_groups.find((g) => g.indexOf(targetId) >= 0);
				if (group) hiddenIds = hiddenIds.filter((id) => group.indexOf(id) >= 0);
			}
			if (hiddenIds.length) {
	...
	*/
	getHiddenTotalDataSum() {
		const { api, state: { hiddenTargetIds } } = this;
		let total = 0;
		if (hiddenTargetIds.size) total = api.data.values.bind(api)([...hiddenTargetIds]).reduce((p, c) => p + c, 0);
		return total;
	},
	/**
	...
	*/
	getMaxDataCount() {
		...
		let max = 0;
		for (let i = 0; i < targets.length; i++) if (targets[i].values.length > max) max = targets[i].values.length;
		return max;
	},
	getMaxDataCountTarget() {
		...
		if (length > 1) {
			...
			for (let i = 0; i < target.length; i++) {
				...
				for (let j = 0; j < values.length; j++) allX.push(values[j].x);
			}
			target = allX;
		...
		} else if (length) target = target[0].values.concat();
		cache.add(KEY.maxDataCountTarget, {
			value: target,
			generation: state.dataGeneration
		});
		return target;
	...
	mapTargetsToUniqueXs(targets) {
		...
		let xs = [];
		if (targets?.length) {
			xs = getUnique(mergeArray(targets.map((t) => t.values.map((v) => +v.x))));
			xs = axis?.isTimeSeries() ? xs.map((x) => /* @__PURE__ */ new Date(+x)) : xs.map(Number);
		}
		return sortValue(xs);
	},
	/**
	...
	*/
	...
	getValuesAsIdKeyed(targets) {
		...
		if (isMultipleX) {
			...
			if (cached && cached.generation === $$.state.dataGeneration) xIndexMap = cached.value;
			else {
		...
		targets.forEach((t) => {
			...
			t.values.filter(({ value }) => isValue(value) || value === null).forEach((v) => {
				let { value } = v;
				if (value !== null && $$.isCandlestickType(v)) value = isArray(value) ? value.slice(0, 4) : [
					value.open,
					value.high,
					value.low,
					value.close
				];
				if (isArray(value)) data.push(...value);
				else if (isObject(value) && "high" in value) data.push(...Object.values(value));
				else if ($$.isBubbleZType(v)) data.push(hasAxis && $$.getBubbleZData(value, "y"));
				else if (isMultipleX && xIndexMap) {
					const xKey = isString(v.x) ? v.x : +v.x;
					const index = xIndexMap.get(xKey);
					if (index !== void 0) data[index] = value;
	...
	*/
	getSortCompareFn(isReversed = false) {
		...
		} else if (isFunction(order)) fn = order.bind($$.api);
		return fn || null;
	},
	filterByX(targets, x) {
	...
	filterNullish(data) {
		...
		return data ? data.filter((v) => "value" in v ? filter(v) : v.values.some(filter)) : data;
	},
	...
	*/
	getDataIndexFromEvent(event) {
		const { $el, config, state: { hasRadar, inputType, eventReceiver: { coords, rect } } } = this;
		let index;
		if (hasRadar) {
			let target = event.target;
			if (/tspan/i.test(target.tagName)) target = target.parentNode;
			...
			} else point -= isRotated ? rect.top : rect.left;
	...
	getDataLabelLength(min, max, key) {
		...
		return $$.getTextRect(values)?.map((rect) => rect[key] * paddingCoef) || [0, 0];
	},
	isNoneArc(d) {
	...
	*/
	getValuesByX(targets) {
		...
		if (cached && cached.generation === state.dataGeneration && cached.targetKey === targetKey) return cached.value;
	...
	*/
	getClosestCandidates(values, pos, useSortedIndex = true) {
		...
		const first = values[0];
		if (!useSortedIndex || len < 200 || !first || !config.data_xSort) return values;
		const isBar = $$.isBarType(first.id);
		const isCandle = $$.isCandlestickType(first.id);
		const sensitivity = config.point_sensitivity;
		if (!(isBar || isCandle) && !isNumber(sensitivity)) return values;
		...
		while (start < end) {
			...
			if (isAscending ? x < pointerX : x > pointerX) start = mid + 1;
			else end = mid;
		}
		const candidates = [];
		...
		const addSameX = (index) => {
			...
			let i = index;
			while (i >= 0 && $$.getXCacheKey(values[i].x) === x) add(i--);
			i = index + 1;
			while (i < len && $$.getXCacheKey(values[i].x) === x) add(i++);
		};
		if (isBar || isCandle) for (let i = start - 2; i <= start + 2; i++) addSameX(i);
		else {
			const maxDx = sensitivity;
			const scan = (index, direction) => {
				for (let i = index; i >= 0 && i < len; i += direction) {
					const v = values[i];
					if (Math.abs(xScale(v.x) - pointerX) > maxDx) break;
					add(i);
				}
			};
			scan(start, 1);
	...
	findClosest(values, pos, useSortedIndex = true) {
		...
		for (let i = 0; i < data.length; i++) {
			...
			if (isBar || isCandle) {
				...
				if (!closest && $$.isWithinBar(main.select(selector).node())) closest = v;
			} else {
				const d = $$.dist(v, pos);
				if (d < $$.getPointSensitivity(v) && (minDist === void 0 || d < minDist)) {
					minDist = d;
					closest = v;
				}
			}
	...
	dist(data, pos) {
		...
		const dx = (scale.zoom || scale.x)(data.x) - pos[xIndex];
	...
	*/
	...
	getRangedData(d, key = "", type = "areaRange") {
		const value = d?.value;
		if (isArray(value)) {
			if (type === "bar") return value.reduce((a, c) => c - a);
			else {
				const index = rangedDataKeyIndex[type]?.[key] ?? -1;
				return index >= 0 ? value[index] : void 0;
			}
		} else if (value && key) return value[key];
		return value;
	},
	/**
	...
	*/
	getRatio(type, d, asPercent = false) {
		...
		if (d && api.data.shown().length) {
			...
			if (type === "arc") {
				if ($$.pie.padAngle()()) ratio = d.value / $$.getTotalDataSum(true);
				else {
				...
				const total = $$.getVisibleTotalPerIndex($$.isStackNormalizedPerGroup() ? d.id : void 0);
				if (total === null) return ratio;
				const divisor = total[d.index];
				d.ratio = isNumber(d.value) && total && divisor ? d.value / divisor : 0;
				ratio = d.ratio;
			} else if (type === "radar") ratio = parseFloat(String(Math.max(d.value, 0))) / state.current.dataMax * config.radar_size_ratio;
			else if (type === "bar") {
				const max = $$.getYScaleById.bind($$)(d.id).domain().reduce((a, c) => c - a);
				ratio = max === 0 ? 0 : Math.abs($$.getRangedData(d, null, type) / max);
			} else if (type === "treemap") ratio /= $$.getTotalDataSum(true);
		}
	...
	updateDataIndexByX(tickValues) {
		...
		$$.data.targets.forEach((t) => {
			t.values.forEach((value, valueIndex) => {
				...
				if (index === void 0) index = valueIndex;
				value.index = index;
			});
		});
	},
	...
	*/
	isBarRangeType(d) {
		const $$ = this;
		const { value } = d;
		return $$.isBarType(d) && !$$.isSubchartSourceTypeOf?.(d, TYPE.CANDLESTICK) && isArray(value) && value.length >= 2 && value.every(isNumber);
	},
	/**
