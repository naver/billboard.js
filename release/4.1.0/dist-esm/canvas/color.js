...
function withOpacity(color, opacity) {
	if (!isNumber(opacity) || opacity >= 1) return color;
	const hex = color.match(/^#([\da-f]{3}|[\da-f]{6})$/i);
	if (hex) {
		const matches = (hex[1].length === 3 ? hex[1].split("").map((v) => v + v).join("") : hex[1]).match(/.{2}/g);
		if (!matches) return color;
		return `rgba(${matches.map((v) => parseInt(v, 16)).join(",")},${opacity})`;
	}
	if (/^rgb\(/i.test(color)) return color.replace(/^rgb\((.*)\)$/i, `rgba($1,${opacity})`);
	if (/^rgba\(/i.test(color)) return color.replace(/^rgba\((.*),\s*([\d.]+)\)$/i, (_, rgb, alpha) => `rgba(${rgb},${Number(alpha) * opacity})`);
	const parsed = parseCanvasColor(color);
	if (parsed && parsed !== color) return withOpacity(parsed, opacity);
	return color;
}
/**
...
function parseCanvasColor(color) {
	...
	if (colorParserContext === void 0) colorParserContext = win.document.createElement("canvas").getContext("2d");
	...
	colorParserContext.fillStyle = color;
	return parsedOnBlack === colorParserContext.fillStyle ? parsedOnBlack : null;
}
//#endregion
