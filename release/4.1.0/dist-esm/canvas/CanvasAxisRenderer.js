...
import { getAdditionalAxisScale, getAdditionalAxisTickFormat, getAdditionalAxisTickValues, getSubXTickLineValues, getSubXTickValues, getXScale, getXTickLinePosition, getXTickLineValues, getXTickValues, getYGridTickValues, getYTickValues, isSameTickValue, normalizeXValue, normalizeYValue } from "./axisTicks.js";
import CanvasPainter from "./CanvasPainter.js";
//#region src/canvas/CanvasAxisRenderer.ts
...
function getSubYTickFormat($$, id) {
	const axisFormat = $$.axis?.[id === "y2" ? "subY2" : "subY"]?.tickFormat?.();
...
function formatAxisTooltipValue($$, id, value) {
	...
	if (scaleValue === null || scaleValue === void 0) return null;
	if (id === "x" && $$.axis?.isTimeSeries?.()) return $$.format.xAxisTick(scaleValue);
...
function getAxisLabelBasePosition($$, id) {
	...
	if (id === "x") !isRotated && (base.y += height);
	else if (id === "y") isRotated && (base.y += height);
	else if (isRotated) base.y -= 1;
	else base.x += width;
	return base;
}
/**
	...
	*/
	function split(lines, value) {
		...
		for (let i = 1; i < value.length; i++) {
			if (value.charAt(i) === " ") spaceIndex = i;
			if (width < charWidth * (i + 1)) return split(lines.concat(value.slice(0, spaceIndex || i)), value.slice(spaceIndex ? spaceIndex + 1 : i));
		}
...
function getXTickTextLines($$, painter, format, tick, ticks = [], isRotated = false, targetScale = getXScale($$), maxWidth) {
	...
	if (text.indexOf("\n") > -1) return text.split("\n");
	return $$.config.axis_x_tick_multiline ? splitTickTextByWidth(text, maxWidth ?? getXTickTextWidth($$, ticks, isRotated, targetScale), painter) : [text];
}
/**
...
function getXTickTextAlign($$, options) {
	...
	if (!isRotated && tickIndex === 0 && (inner === true || inner?.first)) align = "left";
	else if (!isRotated && tickIndex === tickCount - 1 && (inner === true || inner?.last)) align = "right";
	return align;
}
/**
...
function getTitleTextPosition(position, width) {
	if ((position?.indexOf("center") ?? -1) > -1) return {
		x: width / 2,
		align: "center"
	};
	if ((position?.indexOf("right") ?? -1) > -1) return {
		x: width,
		align: "right"
	};
	return {
...
function getYOuterTickDirection(config, isRotated, isY2) {
	if (isRotated) return isY2 ? config.axis_y2_inner ? 1 : -1 : config.axis_y_inner ? -1 : 1;
...
function normalizeXRegionBoundaryValue($$, value) {
	if ($$.axis?.isCategorized?.() && typeof value === "string" && Number.isNaN(Number(value))) return $$.config.axis_x_categories.indexOf(value);
...
function getLineTextPosition(position, start, end) {
	if (position === "start") return start + 4;
...
function getRotatedLineTextPosition(position, start, end) {
	if (position === "start") return end - 4;
	if (position === "middle") return (start + end) / 2;
	return start + 4;
}
/**
...
var CanvasAxisRenderer = class {
	...
	*/
	drawSubXAxis($$) {
		...
		if (!config.subchart_show || !config.subchart_axis_x_show || !scale.subX || width2 <= 0 || height2 <= 0) return;
		const isRotated = config.axis_rotated;
		...
		painter.clipRect(isRotated ? getRotatedXAxisClipRect(margin2, current.width, height2) : {
			...
			ctx.setLineDash([]);
			painter.strokePath(() => {
				if (isRotated) painter.traceLine(x, y1, x, y2);
				else painter.traceLine(x1, y, x2, y);
				if (outerTick) {
					if (isRotated) {
						painter.traceLine(x, y1, x + 6 * outerTickDirection, y1);
						painter.traceLine(x, y2, x + 6 * outerTickDirection, y2);
					} else {
						painter.traceLine(x1, y, x1, y + 6 * outerTickDirection);
						painter.traceLine(x2, y, x2, y + 6 * outerTickDirection);
					}
				}
			});
			...
			if (config.subchart_axis_x_tick_show) for (const tick of lineTicks) {
				const tickPos = scale.subX(normalizeXValue($$, tick));
				const tx = margin2.left + tickPos;
				const ty = margin2.top + tickPos;
				if (!isInAxisRange(isRotated ? ty : tx, rangeStart, rangeEnd)) continue;
				painter.strokePath(() => {
					if (isRotated) painter.traceLine(x, ty, x + 6 * tickDirection, ty);
					else painter.traceLine(tx, y, tx, y + 6 * tickDirection);
				});
			}
	...
	*/
	drawSubYAxes($$) {
		const { config, scale, state: { margin2, width2, height2 } } = $$;
		if (!config.subchart_show || width2 <= 0 || height2 <= 0) return;
		["y", "y2"].forEach((id) => {
			const subScale = id === "y2" ? scale.subY2 : scale.subY;
			if (!config[`subchart_axis_${id}_show`] || !subScale) return;
			this.drawYAxis($$, id, {
				scale: subScale,
				ticks: getYTickValues($$, id, void 0, true, subScale, `subchart_axis_${id}`),
				format: getSubYTickFormat($$, id),
				index: 0,
				outerTick: config[`subchart_axis_${id}_tick_outer`] ?? config[`axis_${id}_tick_outer`],
				margin: margin2,
				width: width2,
				height: height2,
	...
	drawAxisLabels($$) {
		...
		painter.withState(() => {
			...
			ids.forEach((id) => {
				...
				if (!text || !config[`axis_${id}_show`] || id === "y2" && !$$.scale.y2) return;
				const isRotatedLabel = id === "x" && config.axis_rotated || id !== "x" && !config.axis_rotated;
				const { x, y } = getAxisLabelCanvasPosition(getAxisLabelBasePosition($$, id), axis.xForAxisLabel(id) + resolveTextOffset(axis.dxForAxisLabel(id), fontSize), resolveTextOffset(axis.dyForAxisLabel(id), fontSize), isRotatedLabel);
				...
				painter.text(String(text), x, y, { angle: isRotatedLabel ? -90 : 0 });
			});
		});
	}
	/**
	...
	*/
	drawGrid($$) {
		...
		painter.withState(() => {
			...
			if (config.grid_x_show && scale.x) painter.strokePath(() => {
				for (const tick of getXTickValues($$)) {
					const pos = getXPosition($$, tick);
					if (!isDrawable(pos)) continue;
					if (isRotated) painter.traceCrispLine(x1, margin.top + pos, x2, margin.top + pos, grid.lineWidth);
					else painter.traceCrispLine(margin.left + pos, y1, margin.left + pos, y2, grid.lineWidth);
				}
			});
			if (config.grid_y_show && scale.y) painter.strokePath(() => {
				for (const tick of getYGridTickValues($$)) {
					const value = normalizeYValue($$, tick);
					const pos = scale.y(value);
					if (!isDrawable(pos)) continue;
					if (isRotated) painter.traceCrispLine(margin.left + pos, y1, margin.left + pos, y2, grid.lineWidth);
					else painter.traceCrispLine(x1, margin.top + pos, x2, margin.top + pos, grid.lineWidth);
				}
			});
	...
	drawRegions($$) {
		...
		painter.clipRect({
			...
			for (const region of regions) {
				...
				if (!rect || !isDrawable(rect.x) || !isDrawable(rect.y) || !rect.w || !rect.h) continue;
				...
				if (region.label?.text) {
					...
					let tx = x + (label.x || 0);
					let ty = y + (label.y || 0);
					if (center.indexOf("x") > -1) tx += (w - textWidth) / 2;
					if (center.indexOf("y") > -1) ty += (h - lineHeight) / 2;
	...
	drawGridLines($$) {
		...
		painter.withState(() => {
			...
			const drawLine = (line, axisId, draw) => {
				const style = getGridLineCanvasStyle(this.theme.getGridLineStyle(axisId, line, "line"));
			...
			const drawXLine = (line) => {
				if (line.value === void 0 || !scale.x) return;
				const pos = getXPosition($$, line.value);
				if (!isDrawable(pos)) return;
				if (isRotated) {
					...
					drawLabel(line.text, getLineTextPosition(line.position, x1, x2), y - 5, false, getTextStyle("x", line));
				} else {
					...
					ctx.textAlign = line.position === "start" ? "left" : line.position === "middle" ? "center" : "right";
					drawLabel(line.text, x - 5, getRotatedLineTextPosition(line.position, y1, y2), true, getTextStyle("x", line));
				}
			};
			const drawYLine = (line) => {
				const targetScale = line.axis === "y2" ? scale.y2 : scale.y;
				const axisId = line.axis === "y2" ? "y2" : "y";
				if (line.value === void 0 || !targetScale) return;
				const pos = targetScale(normalizeYValue($$, line.value, axisId));
				if (!isDrawable(pos)) return;
				if (isRotated) {
					const x = margin.left + pos;
					drawLine(line, "y", () => {
					...
					ctx.textAlign = line.position === "start" ? "left" : line.position === "middle" ? "center" : "right";
					drawLabel(line.text, x - 5, getRotatedLineTextPosition(line.position, y1, y2), true, getTextStyle("y", line));
				} else {
					const y = margin.top + pos;
					drawLine(line, "y", () => {
						painter.traceLine(x1, y, x2, y);
					...
					ctx.textAlign = line.position === "start" ? "left" : line.position === "middle" ? "center" : "right";
					drawLabel(line.text, getLineTextPosition(line.position, x1, x2), y - 5, false, getTextStyle("y", line));
				}
			};
	...
	*/
	drawAdditionalAxes($$) {
		[
			...
			if (!axesConfig.length || !$$.scale[id] || !$$.config[`axis_${id}_show`]) return;
	...
	*/
	drawXAxis($$, axisOptions) {
		...
		const lineTicks = axisOptions?.ticks || getXTickLineValues($$, ticks, axis.tickWidth);
		...
		const tickTextPosition = config.axis_x_tick_text_position;
		const tickRotate = !isRotated ? $$.getAxisTickRotate?.("x") || 0 : 0;
		painter.clipRect(isRotated ? getRotatedXAxisClipRect(margin, current.width, height) : getHorizontalXAxisClipRect(margin, width, current.height), () => {
			ctx.strokeStyle = axis.lineColor;
			ctx.lineWidth = axis.lineWidth;
			painter.strokePath(() => {
				if (isRotated) painter.traceLine(x, y1, x, y2);
			...
			ctx.strokeStyle = axis.tickColor;
			ctx.lineWidth = axis.tickWidth;
			if (config.axis_x_tick_show) painter.strokePath(() => {
			...
			if (!axisOptions && !config.axis_x_tick_text_show) return;
			const tickFont = getAxisTickFont(axis, "x");
	...
	*/
	drawFocusedXAxisTick($$, focusData) {
		...
		if (focusX === void 0 || !axisInstance || !config.axis_x_show || !config.axis_x_tick_text_show || !axis.activeLabelColor || axis.activeLabelColor === axis.labelColor) return;
	...
	*/
	drawAxisTooltip($$, point) {
		...
		if (!config.axis_tooltip || !point) return;
		const isRotated = config.axis_rotated;
		...
		const drawLabel = (id, scaleValue, x, y, textAlign) => {
			...
			const ascent = metrics.actualBoundingBoxAscent || lineHeight * .8;
			const descent = metrics.actualBoundingBoxDescent || lineHeight * .2;
			const paddingX = Math.max(2, textWidth * .15);
			const paddingY = Math.max(3, lineHeight * .25);
		...
		painter.withState(() => {
			...
			ctx.setLineDash([]);
			painter.strokePath(() => {
				if (isInXRange) painter.traceLine(absX, margin.top, absX, margin.top + height);
				if (isInYRange) painter.traceLine(margin.left, absY, margin.left + width, absY);
			});
			ctx.setLineDash([]);
			if (isRotated) {
				isInYRange && config.axis_x_show && drawLabel("x", localY, margin.left - fontSize * .3, absY + fontSize * .4, "right");
				isInXRange && config.axis_y_show && drawLabel("y", localX, absX - fontSize * 1.3, margin.top + height + fontSize * 1.15, "left");
				isInXRange && config.axis_y2_show && $$.scale.y2 && drawLabel("y2", localX, absX - fontSize * 1.3, margin.top - fontSize * .4, "left");
			} else {
				isInXRange && config.axis_x_show && drawLabel("x", localX, absX - fontSize, margin.top + height + fontSize * 1.15, "left");
				isInYRange && config.axis_y_show && drawLabel("y", localY, margin.left - fontSize * .4, absY + fontSize * .3, "right");
				isInYRange && config.axis_y2_show && $$.scale.y2 && drawLabel("y2", localY, margin.left + width + fontSize * .4, absY + fontSize * .3, "left");
			}
		});
	}
	/**
	...
	*/
	drawXAxisTickText($$, tick, format, fill, options) {
		...
		const tx = margin.left + tickPos;
		const ty = margin.top + tickPos;
		if (!isInAxisRange(isRotated ? ty : tx, rangeStart, rangeEnd)) return;
		...
		const lineHeight = options.tickLineHeight ?? getXTickTextLineHeight(painter, getFontSize(tickFont));
		let textX;
		let textY;
		if (isRotated) {
			textX = x + 9 * tickTextDirection + (tickTextPosition.x || 0);
			...
			const firstDy = tickTextPosition.y ? resolveTextOffset(tickTextPosition.y, fontSize) : .71 * fontSize;
	...
	*/
	drawYAxis($$, id = "y", axisOptions) {
		...
		const x = painter.crisp(plotMargin.left + (isY2 ? plotWidth + (isRotated ? 0 : axisOffset) : -axisOffset), axis.lineWidth);
		...
		const lineTicks = axisOptions?.ticks || (config[`${prefix}_tick_culling`] && config[`${prefix}_tick_culling_lines`] !== false ? getYTickValues($$, id, void 0, false) : ticks);
		const format = axisOptions?.format || $$.axis?.[id]?.tickFormat?.() || config[`${prefix}_tick_format`]?.bind($$.api) || ((v) => v);
		...
		const tickTextPosition = axisOptions?.tickTextPosition || config[`${prefix}_tick_text_position`] || config[`axis_${id}_tick_text_position`];
		const tickShow = axisOptions?.tickShow ?? (axisOptions ? true : config[`${prefix}_tick_show`]);
		const tickTextShow = axisOptions?.tickTextShow ?? (axisOptions ? true : config[`${prefix}_tick_text_show`]);
		painter.withState(() => {
			ctx.strokeStyle = axis.lineColor;
			ctx.lineWidth = axis.lineWidth;
			painter.strokePath(() => {
				if (isRotated) painter.traceLine(x1, y, x2, y);
				else painter.traceLine(x, y1, x, y2);
				if (outerTick) {
					if (isRotated) {
						painter.traceLine(x1, y, x1, y + 6 * outerTickDirection);
						painter.traceLine(x2, y, x2, y + 6 * outerTickDirection);
			...
			const addDrawableTick = (tick, target) => {
				...
				if (!isDrawable(isRotated ? tx : ty)) return;
				target.push({
					tick,
					tx,
					ty
			...
			for (const tick of ticks) addDrawableTick(tick, drawableTicks);
			for (const tick of lineTicks) addDrawableTick(tick, drawableLineTicks);
			if (tickShow) painter.strokePath(() => {
				for (const { tx, ty } of drawableLineTicks) if (isRotated) painter.traceLine(tx, y, tx, y + 6 * tickDirection);
				else painter.traceLine(x, ty, x + 6 * tickDirection, ty);
			});
			if (tickTextShow) for (const { tick, tx, ty } of drawableTicks) if (isRotated) painter.text(formatTick(format, tick), tx + (tickTextPosition.x || 0), y + 6 * tickTextDirection + (tickTextPosition.y || 0));
			else painter.text(formatTick(format, tick), x + 9 * tickTextDirection + (tickTextPosition.x || 0), ty + (tickTextPosition.y || 0));
		});
	}
};
//#endregion
