 ...
 * @version 4.1.0
*/
import { SUBCHART_BRUSH_HANDLE_PATH, TYPE } from "../config/const.js";
import { window as win } from "../module/browser.js";
import { asHalfPixel, isFunction, isNumber, isObject, isString } from "../module/util/type-checks.js";
...
import { getBarRadiusInfo, getBarRadiusResolver, getStackingBarRadiusSet } from "../ChartInternal/shape/core/barRadius.js";
import { getRenderDataPoint, getRenderPoint, getShapePoint, getTreemapLabelText, getTreemapNodeRect } from "../ChartInternal/shape/core/geometry.js";
import { isContinuousGridFocusEnabled } from "../ChartInternal/internals/subchart.util.js";
import { createCanvasPointOccupancyGrid, getCanvasShapeIndices, getCanvasTargetVisibleRange, getFontSize, hasCanvasDrawableValue, isCanvasAreaType, isCanvasBarType, isCanvasBubbleType, isCanvasCandlestickType, isCanvasLineType, isCanvasPointType, isCanvasScatterType, isCanvasTargetSupported, isCanvasTreemapType, isFiniteCanvasCoordinate, markCanvasPointOccupancy } from "./util.js";
import CanvasPainter from "./CanvasPainter.js";
import { withOpacity } from "./color.js";
import { getCanvasBarGeometry, getCanvasCandlestickGeometry } from "./geometry.js";
import { drawLabelDecorations, getExpandedFocusMatcher, getLabelColor, getLabelDecorationBox, getLabelImageOption, getLabelImagePosition, getLabelImageUrl, getLabelPosition, getLabelText, getPointLabelAnchor, getRotatedLabelPosition } from "./labels.js";
...
function getBackgroundImageOffset($$) {
	const { state } = $$;
	return state.hasFunnel || state.hasTreemap ? {
		x: 0,
		y: 0
	} : {
...
function getFocusedCanvasDatum(focusData, d) {
	if (!focusData?.length || !d?.id) return null;
	...
	if (!lookup) {
		...
		for (const focus of focusData) {
			...
			if (!lookup.has(targetKey)) lookup.set(targetKey, focus);
			if ("index" in focus) lookup.set(`${focus.id}:${focus.index}`, focus);
		}
...
function getCanvasOverColor($$, d) {
	...
	if (!onover || !d) return null;
	if (isObject(onover)) return d.id in onover ? onover[d.id] : null;
	else if (isString(onover)) return onover;
	else if (isFunction(onover)) return onover.call($$.api, d);
	return null;
}
/**
...
function getCanvasRenderColor($$, d, focusData) {
	return getCanvasOverColor($$, getFocusedCanvasDatum(focusData, d)) || $$.color(d.id);
}
/**
...
function getCanvasTargetFocusOpacity($$, d) {
	const id = getCanvasTargetId(d);
	return id && $$.state.defocusedTargetIds?.has(id) ? $$.canvasTheme.style.shape.targetDefocusedOpacity : 1;
}
/**
...
function getCanvasAreaBounds($$, target, indices, isSub = false) {
	...
	for (let i = range.start; i < range.end; i++) {
		...
		if (!hasCanvasDrawableValue($$, d)) continue;
		getPoints(d, i).forEach(([x, y]) => {
			if (!isFiniteCanvasCoordinate(x, y)) return;
...
function getCanvasLinearGradientFill($$, ctx, target, shape, rect, baseColor) {
	...
	if (!option || !rect || !rect.w || !rect.h) return baseColor;
	...
	const stops = Array.isArray(gradientOption.stops) ? gradientOption.stops : [[
		0,
		baseColor,
		1
	], [
	...
	stops.forEach(([offset, stopColor, stopOpacity]) => {
		...
		gradient.addColorStop(Number.isFinite(numericOffset) ? Math.max(0, Math.min(1, numericOffset)) : 0, withOpacity(color, stopOpacity));
	});
	return gradient;
}
/**
...
function getPointOpacity($$, target) {
	...
	return Number.isFinite(opacity) ? opacity : isCanvasScatterType($$, target) || isCanvasBubbleType($$, target) ? .5 : 1;
}
/**
...
function shouldCullDenseScatterPoints($$, target, pointType, hasGradient) {
	return !hasGradient && pointType === "circle" && isCanvasScatterType($$, target) && !isFunction($$.config.point_r) && target.values.length > 1e5;
}
/**
...
function drawBarConnectLine($$, painter, type, boxes, alpha = 1) {
	...
	painter.strokePath(() => {
		for (let i = 1; i < boxes.length; i++) {
			...
			if (i < boxes.length - 1) movePoint = getMovePoint(boxes[i]);
		}
	}, {
		alpha,
...
function getPointType($$, target) {
	const { config } = $$;
	if (isCanvasBubbleType($$, target)) return "circle";
	...
	const type = pattern[Math.max(0, targetIds.indexOf(target.id)) % pattern.length];
...
function getPointFillStyle($$, ctx, d, x, y, r, fallback) {
	...
	if (!option || !r) return fallback;
	const gradientOption = isObject(option) ? option : {};
	const cx = isNumber(gradientOption.cx) ? gradientOption.cx : .3;
	const cy = isNumber(gradientOption.cy) ? gradientOption.cy : .3;
	const radius = isNumber(gradientOption.r) ? gradientOption.r : .7;
	const stops = Array.isArray(gradientOption.stops) ? gradientOption.stops : [[
		.1,
		null,
		1
	...
	const gradientX = x + (cx - .5) * r * 2;
	const gradientY = y + (cy - .5) * r * 2;
...
function getCandlestickColor($$, d, value) {
	...
	return (value?._isUp ? $$.color(d) : downColor && typeof downColor === "object" ? downColor[d.id] : downColor) || $$.color(d);
}
/**
...
function drawCanvasLineWithDataRegions($$, target, painter) {
	...
	let values = $$.isAreaRangeType(target) ? rawValues.map((d) => ({
		...d,
		value: $$.getRangedData(d, "mid")
	})) : rawValues;
	if ($$.isStepType(target)) values = $$.convertValuesToStep(values);
...
var CanvasRenderer = class {
	...
	theme;
	painter;
	labelImageCache = /* @__PURE__ */ new Map();
	backgroundImageCache = /* @__PURE__ */ new Map();
	backgroundClassStyleCache = /* @__PURE__ */ new Map();
	/**
	* Constructor.
	...
	*/
	getBackgroundClassStyle($$, className) {
		if (!className || !win.document) return {};
	...
	*/
	drawLabelImage($$, d, text, x, y) {
		...
		if (entry?.loaded) this.ctx.drawImage(entry.image, position.x, position.y, option.width, option.height);
		return {
			x: position.textX,
			y: position.textY
		};
	...
	*/
	drawDataLabel($$, d, text, x, y) {
		...
		painter.textLines(text, x, y, { angle: $$.config.data_labels.rotate });
	}
	/**
	...
	*/
	drawFocusLabels($$, selectedData) {
		if (!$$.hasDataLabel?.()) return;
		const { ctx, theme: { style } } = this;
		const rows = selectedData.filter((d) => d && hasCanvasDrawableValue($$, d) && isCanvasPointType($$, d) && isCanvasRenderableTarget($$, { id: d.id }));
		const texts = { size: () => rows.length };
	...
	*/
	draw($$, shape, focusData) {
		...
		const drawLineLayers = () => {
			if (!$$.config.area_front) this.drawAreas($$, shape, focusData);
			this.drawLines($$, shape);
			if ($$.config.area_front) this.drawAreas($$, shape, focusData);
			this.drawCircles($$, shape, focusData);
		};
		const drawContent = () => {
			if (!$$.config.bar_front) this.drawBars($$, shape, focusData);
	...
	*/
	drawSubchart($$, _shape) {
		const { config, state } = $$;
		if (!config.subchart_show || !state.hasAxis || state.width2 <= 0 || state.height2 <= 0) return;
				...
				painter.clipRect(rect, () => {
					painter.withTranslation(rect.x, rect.y, () => {
						...
						for (const target of areaTargets) {
							if (!target.values.some(hasCanvasDrawableValue.bind(null, $$))) continue;
						...
						if (getBarPoints) {
							for (const target of barTargets) {
								...
								target.values.forEach((d, i) => {
									if (!hasCanvasDrawableValue($$, d)) return;
									const geometry = getCanvasBarGeometry($$, getBarPoints, d, i);
									const fill = $$.getSubchartCandlestickBarColor?.(d, true) || ctx.fillStyle;
						...
						if (config.point_show && !$$.isPointFocusOnly?.()) {
							...
							if (cx && cy) {
								for (const target of targets) {
									if (!isCanvasScatterType($$, target) && !isCanvasBubbleType($$, target)) continue;
									...
									target.values.forEach((d, i) => {
										...
										if (isFiniteCanvasCoordinate(x, y)) drawPointPattern(painter, "circle", x, y, r, pointStyle);
									});
								}
								ctx.globalAlpha = 1;
							}
	...
	*/
	drawSubchartBrush($$) {
		...
		if (!config.subchart_show || config.subchart_brush_enabled === false || !domain?.length || !scale.subX) return;
		const { margin2, width2, height2 } = state;
		...
		painter.withTranslation(margin.left, margin.top, () => {
			for (const target of targets) {
				...
				for (let i = range.start; i < range.end; i++) {
					...
					if (connectLineType) connectLineBoxes.push(getBarConnectLineBox($$, points, radiusInfo));
				}
				if (connectLineType && style.shape.barConnectLineWidth > 0) drawBarConnectLine($$, painter, connectLineType, connectLineBoxes, targetOpacity);
				for (const { d, rect, radiusInfo } of bars) {
					...
					const alpha = (isExpanded(d) ? style.shape.barExpandedOpacity : style.shape.barOpacity) * targetOpacity;
					ctx.fillStyle = overColor ? color : getCanvasLinearGradientFill($$, ctx, target, "bar", rect, color);
					getRadius ? painter.fillRoundRect(rect, radiusInfo.corners, { alpha }) : painter.fillRect(rect, { alpha });
					if (style.shape.barLineWidth > 0) {
						ctx.strokeStyle = style.shape.barStrokeColor;
						ctx.lineWidth = style.shape.barLineWidth;
						getRadius ? painter.strokeRoundRect(rect, radiusInfo.corners, { alpha: style.shape.barOpacity * targetOpacity }) : painter.strokeRect(rect, { alpha: style.shape.barOpacity * targetOpacity });
					}
				}
			}
		});
		...
		painter.withTranslation(margin.left, margin.top, () => {
			...
			for (const target of targets) {
				...
				for (let i = range.start; i < range.end; i++) {
					...
					painter.fillRect(rect, { alpha: (isExpanded(d) ? style.shape.candlestickExpandedOpacity : 1) * targetOpacity });
	...
	*/
	hasExpandedShapeFocus($$, selectedData) {
		return !!selectedData?.some((d) => d && (isCanvasBarType($$, d) || isCanvasCandlestickType($$, d)));
	}
	/**
	...
	*/
	drawLines($$, shape) {
		...
		const { margin } = $$.state;
		if (!$$.generateGetLinePoints) return;
		painter.withTranslation(margin.left, margin.top, () => {
			for (const target of targets) {
				const visibleTarget = getVisibleCanvasTarget($$, target);
				if (!visibleTarget.values.some(hasCanvasDrawableValue.bind(null, $$))) continue;
				ctx.globalAlpha = getCanvasTargetFocusOpacity($$, target);
				ctx.lineWidth = isCanvasTargetFocused($$, target) ? style.shape.lineFocusedWidth : style.shape.lineWidth;
				ctx.strokeStyle = $$.color(target.id);
				$$.config.data_regions?.[visibleTarget.id] ? drawCanvasLineWithDataRegions($$, visibleTarget, painter) : drawCanvasLine($$, visibleTarget, indices, painter);
			}
			ctx.globalAlpha = 1;
		});
	}
	...
	*/
	drawAreas($$, shape, focusData) {
		...
		const { margin } = $$.state;
		if (!$$.generateGetAreaPoints) return;
		painter.withTranslation(margin.left, margin.top, () => {
			for (const target of targets) {
				const visibleTarget = getVisibleCanvasTarget($$, target);
				if (!visibleTarget.values.some(hasCanvasDrawableValue.bind(null, $$))) continue;
				...
				ctx.globalAlpha = style.shape.areaOpacity * getCanvasTargetFocusOpacity($$, target);
				ctx.fillStyle = getCanvasLinearGradientFill($$, ctx, visibleTarget, "area", $$.config.area_linearGradient ? getCanvasAreaBounds($$, target, indices) : null, color);
	...
	*/
	drawCircles($$, shape, focusData) {
		if (!$$.config.point_show || $$.isPointFocusOnly?.()) return;
		const { ctx, painter, theme: { style } } = this;
		...
		painter.withTranslation(margin.left, margin.top, () => {
			for (const target of $$.filterTargetsToShow()) {
				if (!isCanvasPointType($$, target) || !isCanvasRenderableTarget($$, target) || isCanvasLineType($$, target) && !shouldDrawPoints($$, target)) continue;
				const color = $$.color(target.id);
				const pointType = getPointType($$, target);
				const targetOpacity = getCanvasTargetFocusOpacity($$, target);
				const hasGradient = !!$$.config.point_radialGradient;
				const cullDenseScatter = shouldCullDenseScatterPoints($$, target, pointType, hasGradient);
				const range = getCanvasTargetVisibleRange($$, target);
				const visibleCount = range.end - range.start;
				const occupancy = cullDenseScatter ? createCanvasPointOccupancyGrid($$.state.width, $$.state.height, getTargetPointRadius($$, target, target.values[range.start] || target.values[0])) : null;
				...
				if (!hasExpandedFocus && !hasGradient && pointType === "circle") {
					...
					if (visibleCount > MAX_BATCHED_CIRCLE_POINTS) for (let i = range.start; i < range.end; i++) {
						const d = target.values[i];
						if (!hasCanvasDrawableValue($$, d)) continue;
						...
						const x = cx(d, i);
						const y = cy(d, i);
						if (!isFiniteCanvasCoordinate(x, y)) continue;
						if (occupancy && !markCanvasPointOccupancy(occupancy, x, y)) continue;
						ctx.beginPath();
				...
				} else for (let i = range.start; i < range.end; i++) {
					const d = target.values[i];
					if (!hasCanvasDrawableValue($$, d)) continue;
					const expanded = isExpanded(d);
					const baseR = getTargetPointRadius($$, target, d);
					const r = expanded ? $$.pointExpandedR?.(d) ?? baseR * 1.75 : baseR;
					const x = cx(d, i);
					const y = cy(d, i);
					...
					drawPointPattern(painter, pointType, x, y, drawR, hasStroke && !mergeSameColorStroke ? {
						fill: ctx.fillStyle,
						stroke,
						lineWidth
					} : void 0, baseR);
	...
	*/
	drawSelections($$, shape) {
		...
		if (!$$.config.data_selection_enabled || !selectedData?.length || $$.state.hasTreemap) return;
		const { ctx, painter, theme: { style } } = this;
		const { margin } = $$.state;
		const isBar = isCanvasBarType.bind(null, $$);
		const isCandlestick = isCanvasCandlestickType.bind(null, $$);
		const barPoints = selectedData.some(isBar) ? $$.generateGetBarPoints(getCanvasShapeIndices($$, shape, TYPE.BAR, isBar), false) : null;
		const candlestickPoints = selectedData.some(isCandlestick) ? $$.generateGetCandlestickPoints?.(getCanvasShapeIndices($$, shape, TYPE.CANDLESTICK, isCandlestick), false) : null;
		...
		painter.withTranslation(margin.left, margin.top, () => {
			selectedData.forEach((d) => {
				...
				if (isBar(d) && barPoints) {
					...
					getRadius ? painter.fillRoundRect(rect, corners, {
						fill: "#fff",
						alpha: .3
					...
					getRadius ? painter.strokeRoundRect(rect, corners, {
						stroke: color,
						lineWidth: 2
					...
					if (!$$.getCandlestickData?.(d)) return;
					const geometry = getCanvasCandlestickGeometry($$, candlestickPoints, d, d.index);
					if (!geometry) return;
					...
					if (!$$.isPointFocusOnly?.()) drawPointPattern(painter, pointType, x, y, r, {
	...
	*/
	drawLabels($$, shape) {
		...
		const candlestickPoints = $$.generateGetCandlestickPoints ? $$.generateGetCandlestickPoints(getCanvasShapeIndices($$, shape, TYPE.CANDLESTICK, isCanvasCandlestickType.bind(null, $$)), false) : null;
		const targets = $$.filterTargetsToShow().filter((target) => (isCanvasBarType($$, target) || isCanvasPointType($$, target) || isCanvasAreaType($$, target) || isCanvasCandlestickType($$, target)) && isCanvasRenderableTarget($$, target));
		const targetRows = [];
		let labelCount = 0;
		targets.forEach((target) => {
			...
			for (let i = 0; i < data.length; i++) {
				const d = data[i];
				const text = getLabelText($$, d);
				if (text && d.index >= range.start && d.index < range.end && hasCanvasDrawableValue($$, d)) rows.push({
					d,
					text
				});
			}
	...
	*/
	drawEmptyLabel($$) {
		...
		if (!text || targetsToShow.length) return;
		const { margin, width, height } = $$.state;
	...
	*/
	drawZoomBrush($$, start, end) {
		...
		const rect = axis_rotated ? {
			x: margin.left,
			y: margin.top + brushStart,
			w: width,
			h: brushSize
		} : {
			x: margin.left + brushStart,
			y: margin.top,
			w: brushSize,
			h: height
		};
	...
	drawTreemaps($$) {
		...
		painter.withState(() => {
			...
			for (const node of nodes) {
				const { data } = node;
				if (!isCanvasTreemapType($$, data)) continue;
				const { x, y, w, h } = getTreemapNodeRect($$, node, root, true);
				if (!isFiniteCanvasCoordinate(x, y) || !isFiniteCanvasCoordinate(x + w, y + h) || w <= 0 || h <= 0) continue;
				const rect = {
					x,
					y,
					w,
				...
				painter.fillRect(rect);
				painter.strokeRect(rect);
				if (!config.treemap_label_show || (data.ratio || 0) < (config.treemap_label_threshold || 0)) continue;
				...
				drawLabelDecorations($$, painter, data, label, textX, isCentered ? textY + (lines.length - 1) * lineHeight / 2 : blockY);
				lines.forEach((line, i) => {
					painter.text(line, textX, textY + i * lineHeight, { maxWidth: Math.max(0, w - 8) });
				});
			}
		});
	}
	...
	*/
	drawSubchartFocus($$, selectedData) {
		const { config, scale, state } = $$;
		const focus = selectedData?.find((d) => d && hasCanvasDrawableValue($$, d));
		if (!focus || !config.subchart_show || config.subchart_grid_focus_continuous || config.subchart_brush_enabled !== false || config.grid_focus_show === false || config.subchart_grid_focus === false || !config.tooltip_show || config.axis_tooltip || !scale.subX || state.width2 <= 0 || state.height2 <= 0) return;
		const { margin2, width2, height2 } = state;
	...
	drawFocus($$, selectedData) {
		...
		painter.withTranslation(margin.left, margin.top, () => {
			if ($$.config.tooltip_show && $$.config.grid_focus_show !== false && !$$.config.axis_tooltip && focus) {
				...
				if (hasIndexCoordinate) painter.strokePath(() => {
					if (isRotated) {
						painter.traceLine(crispEdgeX(0), y, isEdge && hasValueCoordinate ? x : crispEdgeX(focusEndX), y);
						if (hasValueCoordinate && $$.config.grid_focus_y && !$$.config.tooltip_grouped) {
							const isY2 = $$.axis?.getId(focus.id) === "y2";
							painter.traceLine(x, isEdge && !isY2 ? y : crispEdgeY(0), x, isEdge && isY2 ? y : crispEdgeY($$.state.height));
						}
					} else {
						painter.traceLine(x, isEdge && hasValueCoordinate ? y : crispEdgeY(0), x, crispEdgeY(focusEndY));
						if (hasValueCoordinate && $$.config.grid_focus_y && !$$.config.tooltip_grouped) {
							const isY2 = $$.axis?.getId(focus.id) === "y2";
							painter.traceLine(isEdge && isY2 ? x : crispEdgeX(0), y, isEdge && !isY2 ? x : crispEdgeX($$.state.width), y);
						}
					}
				}, {
					lineDash: style.focusGrid.dashArray,
			...
			if ($$.config.point_show && !$$.state.canvasFocusMainRedraw) selectedData.filter((d) => d && hasCanvasDrawableValue($$, d) && isCanvasPointType($$, d) && isCanvasRenderableTarget($$, { id: d.id })).forEach((d) => {
				const pointType = getPointType($$, { id: d.id });
				const { x, y } = getRenderDataPoint($$, d);
				...
				const mergeSameColorStroke = pointType === "circle" && hasStroke && isNumber(alpha) && alpha < 1 && fill === stroke;
				if (!isFiniteCanvasCoordinate(x, y)) return;
				drawPointPattern(painter, pointType, x, y, mergeSameColorStroke ? r + (lineWidth || 0) / 2 : r, hasStroke && !mergeSameColorStroke ? {
					fill,
					lineWidth,
					stroke,
					alpha
