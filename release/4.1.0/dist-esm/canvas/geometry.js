 ...
 * @version 4.1.0
*/
import { getCandlestickGeometry, getRenderRect } from "../ChartInternal/shape/core/geometry.js";
...
function getCanvasBarGeometry($$, getPoints, d, index) {
	...
	return isFiniteCanvasCoordinate(rect.x, rect.y) && isFiniteCanvasCoordinate(rect.x + rect.w, rect.y + rect.h) ? {
		points,
		rect
	} : null;
}
...
function getCanvasCandlestickGeometry($$, getPoints, d, index) {
	...
	return isFiniteCanvasCoordinate(body.x, body.y) && isFiniteCanvasCoordinate(body.x + body.w, body.y + body.h) && isFiniteCanvasCoordinate(wickStart[0], wickStart[1]) && isFiniteCanvasCoordinate(wickEnd[0], wickEnd[1]) ? {
		body,
		points,
		wickStart,
		wickEnd
