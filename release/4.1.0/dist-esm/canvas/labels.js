...
import { parseShorthand, tplProcess } from "../module/util/object.js";
import { getFontSize, isCanvasBubbleType, isCanvasCandlestickType, isCanvasLineType, isCanvasScatterType } from "./util.js";
//#region src/canvas/labels.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
function getExpandedFocusMatcher($$, selectedData, typeFilter) {
	...
	const keys = new Set(selectedData.filter((d) => d && typeFilter($$, d)).map(getLabelRowKey));
...
function getLabelPosition($$, d, type, texts) {
	...
	let value;
	if (isFunction(position)) value = position.bind($$.api)(type, getLabelValue($$, d), d.id, d.index, texts);
	else if (position) value = (d.id in position ? position[d.id] : position)?.[type];
...
function getLabelImageOption($$, d) {
	...
	if (isFunction(option)) return option.call($$.api, getLabelValue($$, d), d.id, d.index) ?? null;
	else if (option) {
...
function getLabelImageUrl(option, d) {
	return tplProcess(option.url, { ID: d.id });
}
/**
...
function getLabelImagePosition($$, option, text, x, y, d) {
	...
	const imageX = x - w;
	const imageY = y - h - ($$.isTreemapType?.(d) ? fontSize * .7 : textHeight / 2);
	let textX = x;
	let textY = y;
	if ($$.config.axis_rotated) textX += w;
	else textY += h;
	return {
		x: imageX + (pos?.x ?? 0),
		y: imageY + (pos?.y ?? 0),
...
function getLabelColor($$, d, fallback) {
	...
	return typeof color === "string" ? color : $$.isTreemapType?.(d) ? fallback : $$.color?.(d) || fallback;
}
/**
...
* @private
*/
function getLabelDecorationBox(ctx, text, x, y, padding = {
	top: 0,
	right: 0,
	bottom: 0,
	left: 0
	...
	let textX = x;
	let textY = y;
	if (ctx.textAlign === "center") textX -= width / 2;
	else if (ctx.textAlign === "right" || ctx.textAlign === "end") textX -= width;
	if (ctx.textBaseline === "middle") textY -= height / 2;
	else if (ctx.textBaseline === "alphabetic") textY -= height - baselineDescent;
	else if (ctx.textBaseline === "bottom" || ctx.textBaseline === "ideographic") textY -= height;
	return {
...
function drawLabelDecorations($$, painter, d, text, x, y) {
	...
	const box = getLabelDecorationBox(ctx, text, x, y, border?.padding ?? {
		top: 0,
		right: 0,
		bottom: 0,
		left: 0
	...
	painter.withState((canvas) => {
		...
		if (backgroundColor) painter.fillRoundRect(box, border?.radius ?? 0, { fill: backgroundColor });
		if (border) {
			if (border.fill !== "none") painter.fillRoundRect(box, border.radius, { fill: border.fill });
...
function getLabelRotateAnchor(angle) {
	let anchor = "middle";
	if (angle > 0 && angle <= 170) anchor = "end";
	else if (angle > 190 && angle <= 360) anchor = "start";
	return anchor;
}
/**
...
function getRotatedLabelPosition($$, d, x, y) {
	...
	if (isRotated) {
		...
		if (anchor === "start") {
			x += gap;
			isNegative && (y += 16);
		} else if (anchor === "middle") y -= doubleGap;
		else if (anchor === "end") {
			x -= gap;
			isNegative && (y += 16);
		}
...
function getPointLabelAnchor($$, ctx, d, x, y) {
	if ($$.config.axis_rotated) {
		...
		let baseY = 3;
		if (isNumber(config.point_r) && config.point_r > 5 && (isCanvasLineType($$, d) || isCanvasScatterType($$, d))) baseY += config.point_r / 2.3;
		ctx.textAlign = "center";
