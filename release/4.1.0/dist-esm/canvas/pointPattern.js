...
//#region src/canvas/pointPattern.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
const DEFAULT_POINT_VIEWBOX = {
	x: 0,
	y: 0,
	w: 8,
	h: 8
...
function parseSvgPoints(value) {
	...
	const points = [];
	for (let i = 0; i < nums.length - 1; i += 2) points.push([nums[i], nums[i + 1]]);
	return points;
}
/**
...
function mergePointBoxes(boxes) {
	const validBoxes = boxes.filter((box) => Number.isFinite(box.x) && Number.isFinite(box.y) && Number.isFinite(box.w) && Number.isFinite(box.h));
	if (!validBoxes.length) return { ...DEFAULT_POINT_VIEWBOX };
...
function parsePointTransform(value) {
	...
	(value?.match(/[a-z]+\([^)]*\)/gi) || []).forEach((token) => {
		...
		const values = (body?.match(/[-+]?(?:\d+\.?\d*|\.\d+)(?:e[-+]?\d+)?/gi) || []).map(Number).filter(Number.isFinite);
		let next = null;
		if (name === "matrix" && values.length >= 6) next = values.slice(0, 6);
		else if (name === "translate" && values.length) next = [
			1,
			0,
			0,
			1,
		...
		else if (name === "scale" && values.length) next = [
			values[0],
			0,
			0,
			values[1] ?? values[0],
			...
			next = values.length >= 3 ? multiplyPointMatrix(multiplyPointMatrix([
				1,
				0,
				0,
				1,
		...
		} else if (name === "skewx" && values.length) next = [
			1,
			0,
		...
		else if (name === "skewy" && values.length) next = [
			1,
			Math.tan(values[0] * Math.PI / 180),
			0,
			1,
...
function getPointPresentationValue(node, name) {
	...
	return ((style?.match(new RegExp(`(?:^|;)\\s*${name}\\s*:\\s*([^;]+)`, "i")))?.[1] || attr || "").trim() || null;
}
/**
...
function parsePointStyle(node, inherited) {
	...
	if (fill) style.fill = fill === "none" ? null : fill;
	if (stroke) style.stroke = stroke === "none" ? null : stroke;
	if (strokeWidth) {
...
function collectPointPaints(defs) {
	...
	defs.forEach((node, id) => {
		...
		if (tagName === "lineargradient") paints.set(id, {
			type: "linearGradient",
		...
		else if (tagName === "radialgradient") paints.set(id, {
			type: "radialGradient",
...
function parseCustomPointNode(node, fallbackBox, defs, matrix = IDENTITY_POINT_MATRIX, inheritedStyle = {}, seen = /* @__PURE__ */ new Set()) {
	...
	if (tagName === "svg" || tagName === "g" || tagName === "symbol") return children.reduce((shapes, child) => shapes.concat(parseCustomPointNode(child, fallbackBox, defs, transform, style, seen)), []);
	if (tagName === "use") {
		const href = node.getAttribute("href") || node.getAttribute("xlink:href") || "";
		const id = href.charAt(0) === "#" ? href.slice(1) : "";
		const target = id && defs.get(id);
		if (!target || seen.has(id)) return [];
		const useTransform = multiplyPointMatrix(transform, [
			1,
			0,
			0,
			1,
			getSvgNumber(node, "x", 0),
			getSvgNumber(node, "y", 0)
		]);
		seen.add(id);
...
function parseCustomPointPattern(pattern) {
	...
	if (customPointPatternCache.has(pattern)) return customPointPatternCache.get(pattern) || null;
	let parsed = null;
	try {
		const root = new win.DOMParser().parseFromString(sanitize(pattern), "image/svg+xml").documentElement;
...
function createCustomPointPaint(ctx, paint, box) {
	const { x, y, w, h } = box;
	const gradient = paint.type === "linearGradient" ? ctx.createLinearGradient(pointPaintCoord(paint.x1, x, w, x), pointPaintCoord(paint.y1, y, h, y), pointPaintCoord(paint.x2, x, w, x + w), pointPaintCoord(paint.y2, y, h, y)) : ctx.createRadialGradient(pointPaintCoord(paint.fx, x, w, x + w / 2), pointPaintCoord(paint.fy, y, h, y + h / 2), pointPaintCoord(paint.fr, x, Math.max(w, h), 0), pointPaintCoord(paint.cx, x, w, x + w / 2), pointPaintCoord(paint.cy, y, h, y + h / 2), Math.max(.01, pointPaintCoord(paint.r, 0, Math.max(w, h), Math.max(w, h) / 2)));
...
function getCustomPointDrawStyle(ctx, base, shapeStyle, paints, box) {
	...
	if (shapeStyle.fill !== void 0) {
		if (shapeStyle.fill === null) delete style.fill;
		...
		} else if (!/^url\(/.test(shapeStyle.fill)) style.fill = shapeStyle.fill;
	}
	if (shapeStyle.stroke !== void 0) {
		if (shapeStyle.stroke === null) delete style.stroke;
		...
		} else if (!/^url\(/.test(shapeStyle.stroke)) style.stroke = shapeStyle.stroke;
	}
