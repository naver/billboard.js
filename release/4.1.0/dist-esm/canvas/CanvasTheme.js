...
//#region src/canvas/CanvasTheme.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
function toPaintOpacity(value, fallback) {
	const result = toNumber(value.opacity, 1) * toNumber(value["fill-opacity"], 1);
...
function toDashArray(value) {
	if (Array.isArray(value)) return value.filter((v) => Number.isFinite(v) && v > 0);
...
function toFont(style, fallback) {
	...
	if (!fontSize || !fontFamily) return fallback;
...
function readColorValue(style, prop) {
	...
	return (value == null ? "" : toColor(String(value), "")) || void 0;
}
/**
...
function readPaintOpacityValue(style) {
	...
	return opacity === void 0 && fillOpacity === void 0 ? void 0 : (opacity ?? 1) * (fillOpacity ?? 1);
}
/**
...
function readFontValue(style) {
	...
	if (font != null && String(font).trim()) return String(font).trim();
	const fontSize = getStyleValue(style, "font-size");
	const fontFamily = getStyleValue(style, "font-family");
	if (fontSize == null || fontFamily == null) return;
...
function mergeThemeSection(target, section, values) {
	...
	if (Object.keys(override).length) target[section] = mergeObj(target[section] ?? {}, override);
}
/**
...
function normalizeThemeSelectors(selector) {
	return selector.split(",").map((value) => value.trim().replace(/\s*>\s*/g, " ").replace(/\s+/g, " ").replace(/^(?:svg)?\.bb\s+/, "").replace(/(^|\s)(?:g|path|rect|line|text|circle)\./g, "$1.")).filter(Boolean);
}
/**
...
function assignDefined(target, source, skip) {
	getDefinedKeys(source).forEach((key) => {
		if (!skip?.has(key)) target[key] = source[key];
	});
	return target;
}
/**
...
function isSimpleGridLineSelector(selector) {
	...
	return tokens.every((token, index) => token === "line" || token === "text" ? index === tokens.length - 1 : /^(\.[A-Za-z_-][\w-]*)+$/.test(token));
}
/**
...
function getGridLineSelectorStyle(style, target) {
	return assignDefined({}, target === "text" ? {
...
function getGridLineSelectorOverrides(selectors) {
	...
	Object.keys(selectors ?? {}).forEach((selector) => {
		normalizeThemeSelectors(selector).forEach((normalizedSelector) => {
			...
			if (isSimpleGridLineSelector(normalizedSelector) && getDefinedKeys(style).length) overrides.push({
...
function matchesGridLineSelector(selector, axis, className, target) {
	if (!isSimpleGridLineSelector(selector)) return false;
	const selectorTarget = getGridLineSelectorTarget(selector);
	if (selectorTarget && selectorTarget !== target) return false;
	else if (!selectorTarget && target === "text") return false;
...
function applySelectorStyle(target, selector, style) {
	switch (selector) {
		...
		case ".bb-axis-x .tick._active_ text":
			mergeThemeSection(target, "axis", { activeLabelColor: readColorValue(style, "fill") });
			break;
		...
		case ".bb-bar._expanded_":
			mergeThemeSection(target, "shape", { barExpandedOpacity: readPaintOpacityValue(style) });
			break;
		case ".bb-candlestick":
			mergeThemeSection(target, "shape", {
		...
		case ".bb-candlestick._expanded_":
			mergeThemeSection(target, "shape", { candlestickExpandedOpacity: readPaintOpacityValue(style) });
			break;
		case ".bb-line":
			mergeThemeSection(target, "shape", {
		...
		case ".bb-target.bb-focused path.bb-line":
			mergeThemeSection(target, "shape", { lineFocusedWidth: readNumberValue(style, "stroke-width") });
			break;
		case ".bb-target.bb-defocused":
			mergeThemeSection(target, "shape", { targetDefocusedOpacity: readNumberValue(style, "opacity") });
			break;
		case ".bb-area":
			mergeThemeSection(target, "shape", { areaOpacity: readPaintOpacityValue(style) });
			break;
		case ".bb-circle":
			mergeThemeSection(target, "shape", {
		...
		case ".bb-text":
			...
			break;
		case ".bb-title": mergeThemeSection(target, "title", {
			color: readColorValue(style, "fill"),
			font: readFontValue(style)
		});
	}
...
* @private
*/
var CanvasTheme = class {
	style;
	cacheContainer = null;
	cacheKey = null;
	gridLineSelectorOverrides = [];
	directGridOverrideKeys = /* @__PURE__ */ new Set();
	/**
	...
	*/
	load(container, userOverride) {
		...
		this.directGridOverrideKeys = new Set(Object.keys(userOverride?.grid ?? {}));
		...
		const read = (el, props) => {
			...
			const values = {};
			for (const prop of props) values[prop] = prop === "font" ? toFont(style, "") : style.getPropertyValue(prop).trim();
			return values;
		};
		...
		const plainCircle = probe("circle", "", [
			"fill",
			"stroke",
			"stroke-width"
		]);
		const gridLine = probeIn("bb-grid", "line", "bb-ygrid", [
			"stroke",
			"stroke-width",
			"stroke-dasharray"
		]);
		const gridText = probeIn("bb-grid", "text", "", ["fill", "font"]);
		const focusGridLine = probeInNested("bb-grid", "bb-xgrid-focus", "line", "bb-xgrid-focus", [
			"stroke",
			"stroke-width",
			"stroke-dasharray"
		]);
		...
		const candlestickExpanded = probe("path", "bb-candlestick _expanded_", ["opacity", "fill-opacity"]);
		...
		const point = probe("circle", "bb-circle", [
			"fill",
			"stroke",
			"stroke-width"
		]);
		...
		const focusPoint = probeTargetPoint("bb-circle _expanded_", [
			"fill",
			"stroke",
			"stroke-width"
		]);
		const zoomBrush = probe("rect", "bb-zoom-brush", [
			"fill",
			"fill-opacity",
			"opacity"
		]);
		...
		const zoomBrushOpacity = toNumber(zoomBrush["fill-opacity"] || zoomBrush.opacity, .1);
		const subchartBrushFill = toColor(subchartBrushSelection.fill, toColor(subchartBrushExtent.fill, zoomBrushFill));
		const subchartBrushOpacity = toOptionalNumber(subchartBrushSelection["fill-opacity"], 1) ?? toOptionalNumber(subchartBrushSelection.opacity, 1) ?? toOptionalNumber(subchartBrushExtent["fill-opacity"], 1) ?? toOptionalNumber(subchartBrushExtent.opacity, 1) ?? zoomBrushOpacity;
		...
		const defaultStyle = {
			...
			region: {
				...
				opacity: toNumber(regionRect["fill-opacity"], .1),
			...
			shape: {
				...
				areaOpacity: toPaintOpacity(area, .2),
				targetDefocusedOpacity: toNumber(defocusedTarget.opacity, .3),
	...
	*/
	reload(container, userOverride) {
		...
		if (key !== null && this.style && this.cacheContainer === container && this.cacheKey === key) return;
	...
	*/
	getGridLineStyle(axis, line = {}, target = "line") {
		...
		for (const override of this.gridLineSelectorOverrides) {
			if (!matchesGridLineSelector(override.selector, axis, line.class, target)) continue;
