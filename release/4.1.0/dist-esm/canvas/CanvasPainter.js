...
//#region src/canvas/CanvasPainter.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
var CanvasPainter = class {
	...
	*/
	text(text, x, y, style) {
		const fillText = (ctx, textX, textY) => {
			style?.maxWidth === void 0 ? ctx.fillText(text, textX, textY) : ctx.fillText(text, textX, textY, style.maxWidth);
		};
		const draw = (ctx) => {
			if (style?.angle) {
				ctx.translate(x, y);
				...
				fillText(ctx, 0, 0);
			} else fillText(ctx, x, y);
		};
		if (style?.angle) this.withState((ctx) => {
			this.applyStyle(style);
			draw(ctx);
		});
		else this.withStyle(style, draw);
	}
	/**
	...
	textLines(text, x, y, style) {
		this.withState((ctx) => {
			...
			lines.forEach((line, i) => {
				...
				style?.maxWidth === void 0 ? ctx.fillText(line, 0, lineY) : ctx.fillText(line, 0, lineY, style.maxWidth);
			});
		});
	}
	/**
	...
	*/
	point(type, x, y, r, style) {
		this.withStyle(style, (ctx) => {
			const shouldFill = !style?.stroke || style.fill !== void 0;
			const shouldStroke = style?.stroke !== void 0;
			if (type === "rectangle") {
				const size = r * 2;
				const rect = {
					x: x - r,
	...
	*/
	crisp(value, lineWidth) {
		return lineWidth % 2 ? Math.round(value) + .5 : Math.round(value);
	}
};
//#endregion
