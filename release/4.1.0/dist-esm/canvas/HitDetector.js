...
import { createCanvasPointOccupancyGrid, getCanvasShapeIndices, getCanvasTargetVisibleRange, hasCanvasDrawableValue, isCanvasBarType, isCanvasBubbleType, isCanvasCandlestickType, isCanvasPointType, isCanvasScatterType, isCanvasTargetSupported, isCanvasTreemapType, isFiniteCanvasCoordinate, markCanvasPointOccupancy } from "./util.js";
import { getCanvasBarGeometry, getCanvasCandlestickGeometry } from "./geometry.js";
//#region src/canvas/HitDetector.ts
/**
* Copyright (c) 2017 ~ present NAVER Corp.
* billboard.js project is licensed under the MIT license
*/
...
function getGridItems(grid, x, y, size, radius = 0) {
	...
	for (let dx = -radius; dx <= radius; dx++) for (let dy = -radius; dy <= radius; dy++) {
		...
		if (values?.length) for (let i = 0; i < values.length; i++) items.push(values[i]);
	}
	return items;
}
/**
...
function getPointHitSensitivity($$, d) {
	...
	if (Number.isFinite(sensitivity)) return sensitivity;
	$$.pointR?.(d);
...
function shouldCullDenseScatterHitPoints($$, target) {
	return isCanvasScatterType($$, target) && target.values.length > 1e5 && !$$.config.data_selection_enabled && Number.isFinite($$.config.point_sensitivity) && Number.isFinite($$.config.point_r);
}
/**
* Build and query canvas hit-test indexes.
* @private
*/
var HitDetector = class {
	bars = [];
	points = [];
	indices = [];
	barGrid = /* @__PURE__ */ new Map();
	pointGrid = /* @__PURE__ */ new Map();
	plot = {
		x: 0,
		y: 0,
		w: 0,
	...
	*/
	rebuild($$, shape) {
		...
		this.pointBased = !($$.config.axis_x_forceAsSingle && this.grouped) && (!!$$.isMultipleX?.() || targets.some((target) => isCanvasScatterType($$, target) || isCanvasBubbleType($$, target)));
		const indexMap = this.grouped && (!this.pointBased || $$.config.data_selection_enabled && $$.config.data_selection_grouped) ? /* @__PURE__ */ new Map() : null;
		const addIndex = (x, y, data) => {
			if (!indexMap) return;
			const key = $$.getXCacheKey?.(data.x) ?? data.index;
			if (Number.isFinite(data.index) && !indexMap.has(key) && isFiniteCanvasCoordinate(x, y)) indexMap.set(key, {
				x,
				y,
				data
			});
		...
		if ($$.state.hasTreemap) {
			...
			for (const node of nodes) {
				...
				if (!isFiniteCanvasCoordinate(x, y) || !isFiniteCanvasCoordinate(x + w, y + h)) continue;
				this.addBar({
					x,
					y,
					w,
		...
		if (shape.indices[TYPE.CANDLESTICK] || targets.some(isCanvasCandlestickType.bind(null, $$))) {
			...
			if (getPoints) targets.filter(isCandlestick).filter((target) => isCanvasTargetSupported($$, target, HIT_GROUPED_TYPE_FILTERS)).forEach((target) => {
				const range = getCanvasTargetVisibleRange($$, target);
				for (let i = range.start; i < range.end; i++) {
					const d = target.values[i];
					if (!$$.getCandlestickData?.(d)) continue;
		...
		if (cx && cy) targets.filter(isCanvasPointType.bind(null, $$)).filter((target) => isCanvasTargetSupported($$, target, HIT_GROUPED_TYPE_FILTERS)).forEach((target) => {
			const range = getCanvasTargetVisibleRange($$, target);
			...
			for (let i = range.start; i < range.end; i++) {
				...
				if (occupancy && !markCanvasPointOccupancy(occupancy, x - margin.left, y - margin.top)) continue;
				this.addPoint({
					x,
					y,
					sensitivity,
	...
	*/
	hitBar(mx, my) {
		for (const item of getGridItems(this.barGrid, mx, my, BAR_CELL_SIZE)) {
			const { w = 0, h = 0 } = item;
			if (mx >= item.x && mx <= item.x + w && my >= item.y && my <= item.y + h) return item.data;
		}
		return null;
	}
	/**
	...
	*/
	hitPoint(mx, my) {
		...
		for (const item of getGridItems(this.pointGrid, mx, my, this.pointCellSize, 1)) {
			...
			if (dist <= (item.sensitivity ?? HIT_DISTANCE) && dist < min) {
				min = dist;
				nearest = item;
			}
		}
	...
	*/
	buildPointGrid() {
		...
		this.pointGrid = /* @__PURE__ */ new Map();
		for (const point of this.points) addGridItem(this.pointGrid, point, this.pointCellSize);
	}
	/**
	...
	*/
	findNearestIndexItem(x, y) {
		...
		while (start < end) {
			...
			if (indices[mid][indexAxis] < value) start = mid + 1;
			else end = mid;
		}
