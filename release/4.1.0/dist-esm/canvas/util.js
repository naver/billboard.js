...
function isCanvasTargetSupported($$, target, groupedTypeFilters = []) {
	return !$$.isGrouped?.(typeof target === "string" ? target : target?.id) || groupedTypeFilters.some((filter) => filter($$, target));
}
/**
...
function lowerBoundCanvasRows(values, target, ascending) {
	...
	while (low < high) {
		...
		if (ascending ? value < target : value > target) low = mid + 1;
		else high = mid;
	}
	return low;
}
...
function upperBoundCanvasRows(values, target, ascending) {
	...
	while (low < high) {
		const mid = low + high >> 1;
		const value = getComparableRowX(values[mid]);
		if (value === null) return values.length;
		if (ascending ? value <= target : value >= target) low = mid + 1;
		else high = mid;
	}
	return low;
...
function getCanvasTargetVisibleRange($$, target, padding = 1) {
	...
	if (!values.length || !domain) return fullRange;
	const state = $$.state;
...
function markCanvasPointOccupancy(grid, x, y) {
	...
	if (px < 0 || px >= grid.width || py < 0 || py >= grid.height) return false;
	const index = py * grid.width + px;
	if (grid.data[index]) return false;
	grid.data[index] = 1;
	return true;
}
//#endregion
