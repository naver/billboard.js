...
function checkModuleImport(ctx) {
	...
	const { config } = $$;
	let type = "";
	if (isEmpty(config.data_type || config.data_types) && !$$[TYPE_METHOD_NEEDED.LINE]) type = "line";
	else for (const x in TYPE_METHOD_NEEDED) {
		const t = TYPE[x];
	...
	type && logError(`Please, make sure if %c${camelize(type)}`, "module has been imported and specified correctly.", MODULE_IMPORT_DOC);
}
/**
...
function checkApiModuleImport(apiName) {
	...
	moduleName && logError(`Please, make sure if %c${moduleName}`, "module has been imported and specified correctly.", MODULE_IMPORT_DOC);
}
/**
