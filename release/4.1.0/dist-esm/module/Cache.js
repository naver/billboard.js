...
var Cache = class {
	...
	*/
	remove(key) {
		...
		for (let i = 0; i < keys.length; i++) this.cache.delete(keys[i]);
	}
	/**
	* Get cahce
	* @param {string|Array} key Cache key
	...
	*/
	get(key, isDataType = false) {
		if (isDataType && Array.isArray(key)) {
			const targets = [];
			for (let i = 0, id; id = key[i]; i++) if (this.cache.has(id)) targets.push(this.cache.get(id));
			return targets;
		} else return this.cache.has(key) ? this.cache.get(key) : null;
	}
	/**
	...
	*/
	reset(all, excludePrefixes) {
		...
		else this.cache.forEach((_, x) => {
			if (/^\$/.test(x)) {
				if (!excludePrefixes?.some((prefix) => x.startsWith(prefix))) this.cache.delete(x);
			}
		});
	}
	/**
