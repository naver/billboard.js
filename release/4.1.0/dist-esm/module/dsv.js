		...
		*/
		function token() {
			...
			if (text.charCodeAt(start) === QUOTE) {
				while (++pos < len) if (text.charCodeAt(pos) === QUOTE) {
					if (text.charCodeAt(pos + 1) !== QUOTE) break;
					pos++;
				}
				if ((end = pos++) >= len) eof = true;
				else if ((c = text.charCodeAt(pos++)) === NEWLINE) eol = true;
				else if (c === RETURN) {
					eol = true;
			...
			while (pos < len) {
				if ((c = text.charCodeAt(end = pos++)) === NEWLINE) eol = true;
				else if (c === RETURN) {
					eol = true;
					text.charCodeAt(pos) === NEWLINE && ++pos;
