...
import { isArray, isDefined, isFunction, isNumber, isObject, isString, notEmpty } from "./type-checks.js";
...
function _forEachValidItem(items, callback) {
	for (let i = 0; i < items.length; i++) {
		const item = items[i];
		if (item !== null && isDefined(item)) callback(item, i);
	}
}
/**
...
function hasValue(dict, value) {
	for (const key in dict) if (dict[key] === value) return true;
	return false;
}
/**
...
function endall(transition, cb) {
	...
	if ("duration" in transition) transition.each(() => ++n).on("end", end);
	else {
		++n;
		transition.call(end);
	}
...
function deepClone(...objectN) {
	...
	return objectN.map((v) => clone(v)).reduce((a, c) => ({
		...a,
		...c
	}));
}
...
function extend(target = {}, source) {
	if (isArray(source)) source.forEach((v) => extend(target, v));
	for (const p in source) {
		if (/^\d+$/.test(p) || p in target) continue;
		target[p] = source[p];
	}
	return target;
}
...
function sortValue(data, isAsc = true) {
	let fn;
	if (data[0] instanceof Date) fn = isAsc ? (a, b) => a - b : (a, b) => b - a;
	else if (isAsc && !data.every(isNaN)) fn = (a, b) => a - b;
	else if (!isAsc) fn = (a, b) => a > b && -1 || a < b && 1 || a === b && 0;
...
function getMinMax(type, data) {
	...
	if (res.length) {
		if (isNumber(res[0])) {
			...
			for (const v of res) if (type === "min" ? v < result : v > result) result = v;
			res = result;
		} else if (res[0] instanceof Date) res = sortValue(res, type === "min")[0];
	} else res = void 0;
	return res;
}
/**
...
function parseShorthand(value) {
	...
	const [a, b = a, c = a, d = b] = (isString(value) ? value.trim().split(/\s+/) : [value]).map((v) => +v || 0);
	return {
		top: a,
		right: b,
		bottom: c,
...
function runUntil(fn, conditionFn) {
	if (conditionFn() === false) requestAnimationFrame(() => runUntil(fn, conditionFn));
	else fn();
}
/**
